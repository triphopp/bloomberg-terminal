...
function num(args: ResolvedArgs, idx: number): number | undefined {
  return (args.positional.filter((a) => a.type === "number")[idx] as { type: "number"; value: number } | undefined)?.value;
}
...
async function analytics(
  ...
  const q = new URLSearchParams({ fn, ...Object.fromEntries(Object.entries(params).map(([k, v]) => [k, String(v)])) });
...
const NAV: CommandDef[] = [
  { name: "MKT",   group: "nav", description: "Go to Market view",   handler: (_, ctx) => { ctx.setView("market");    return { kind: "navigate", view: "market" }; } },
  { name: "NEWS",  group: "nav", description: "Go to News view",     handler: (_, ctx) => { ctx.setView("news");      return { kind: "navigate", view: "news" }; } },
  { name: "GMOV",  group: "nav", description: "Go to Market Movers", handler: (_, ctx) => { ctx.setView("movers");    return { kind: "navigate", view: "movers" }; } },
  { name: "CLIP",  group: "nav", description: "Go to Clippings / AI",handler: (_, ctx) => { ctx.setView("clippings"); return { kind: "navigate", view: "clippings" }; } },
  { name: "MACRO", group: "nav", description: "Go to Macro Economics",handler: (_, ctx) => { ctx.setView("macro");    return { kind: "navigate", view: "macro" }; } },
  { name: "CRDT",  group: "nav", description: "Go to Credit view",   handler: (_, ctx) => { ctx.setView("credit");    return { kind: "navigate", view: "credit" }; } },
  { name: "PORT",  group: "nav", description: "Go to Portfolio",     handler: (_, ctx) => { ctx.setView("portfolio"); return { kind: "navigate", view: "portfolio" }; } },
  { name: "CRYP",  group: "nav", description: "Go to Crypto view",   handler: (_, ctx) => { ctx.setView("crypto");   return { kind: "navigate", view: "crypto" }; } },
  { name: "FX",    group: "nav", description: "Go to FX / Forex view",handler: (_, ctx) => { ctx.setView("fx");      return { kind: "navigate", view: "fx" }; } },
];
...
const SETTINGS: CommandDef[] = [
  {
    name: "ALERT ON", group: "setting", description: "Show Bloomberg ticker crawl",
    handler: (_, ctx) => { ctx.setTickerEnabled(true); return { kind: "action" }; },
  },
  {
    name: "ALERT OFF", group: "setting", description: "Hide Bloomberg ticker crawl",
    handler: (_, ctx) => { ctx.setTickerEnabled(false); return { kind: "action" }; },
  },
  {
    name: "ALERT CLEAR", group: "setting", description: "Clear regime change alert events",
  ...
  {
    name: "DARK",  group: "setting", description: "Switch to dark mode",
    handler: (_, ctx) => { ctx.setDarkMode(true);  return { kind: "action" }; },
  },
  {
    name: "LIGHT", group: "setting", description: "Switch to light mode",
    handler: (_, ctx) => { ctx.setDarkMode(false); return { kind: "action" }; },
  },
  {
    name: "YTD ON",  group: "setting", description: "Show YTD % column",
    handler: (_, ctx) => { ctx.setShowYTD(true);  return { kind: "action" }; },
  },
  {
    name: "YTD OFF", group: "setting", description: "Show Daily % column",
    handler: (_, ctx) => { ctx.setShowYTD(false); return { kind: "action" }; },
  },
];
...
const INFO: CommandDef[] = [
  {
    name: "REGIME", group: "info", description: "Show current correlation regime",
  ...
  {
    name: "HELP", group: "info", description: "List all terminal commands",
    handler: () => ({ kind: "stay" } as CommandResult),
  },
  {
    name: "PING", group: "info", description: "Check backend status",
    handler: async (_, _ctx, signal) => {
      const t0  = Date.now();
...
const ANALYSIS: CommandDef[] = [
    ...
    handler: async (args, _, signal) => {
      ...
      try {
        const d = await analyticsJson<{ beta: number; alpha: number; r2: number; n: number }>(
          "beta", { asset, benchmark: bench, period: p }, signal,
        );
        return {
          kind: "display",
          content: {
    ...
    handler: async (args, _, signal) => {
      ...
      try {
        const d = await analyticsJson<{
          return: number; start_price: number; end_price: number;
    ...
    handler: async (args, _, signal) => {
      ...
      try {
        const d = await analyticsJson<{ max_drawdown: number; trough_date: string }>(
          "drawdown", { symbol: s, period: p }, signal,
        );
        return {
          kind: "display",
          content: {
  ...
  // ── sharpe(symbol, period?) ───────────────────────────────────────────────
  {
    ...
    handler: async (args, _, signal) => {
      ...
      try {
        const d = await analyticsJson<{ sharpe: number; annualised_return: number; annualised_vol: number }>(
  ...
  // ── zscore(symbol, period?) ───────────────────────────────────────────────
  {
    ...
    handler: async (args, _, signal) => {
      ...
      try {
        const d = await analyticsJson<{ zscore: number; current: number; mean: number; std: number }>(
  ...
  // ── compare(A, B, C, ..., period?) ────────────────────────────────────────
  {
    ...
    handler: async (args, _, signal) => {
      ...
      try {
        const d = await analyticsJson<{ rows: Array<{ symbol: string; return: number; vol: number; sharpe: number; drawdown: number }> }>(
        ...
        return {
          ...
          content: {
            ...
            rows:  d.rows.map((r) => ({
              cells:  [r.symbol, fmtPct(r.return), fmtPct(r.vol), fmtNum(r.sharpe, 2), fmtPct(r.drawdown)],
  ...
  // ── rank(A, B, C, by=return, period?) ────────────────────────────────────
  {
    ...
    handler: async (args, _, signal) => {
      ...
      if (filterSym.length < 1) return { kind: "error", message: "Usage: rank(A, B, C, SHARPE, period?)" };
      try {
        const d = await analyticsJson<{ rows: Array<{ symbol: string; return: number; vol: number; sharpe: number; drawdown: number }>; by: string }>(
        ...
        return {
          ...
          content: {
            ...
            cols:  ["#", "SYMBOL", "RETURN", "VOL", "SHARPE", "DRAWDOWN"],
            rows:  d.rows.map((r, i) => ({
              cells:  [`${i + 1}`, r.symbol, fmtPct(r.return), fmtPct(r.vol), fmtNum(r.sharpe, 2), fmtPct(r.drawdown)],
              colors: ["", "accent", rowColor(r.return), "", rowColor(r.sharpe), rowColor(r.drawdown)],
            })),
          },
        };
