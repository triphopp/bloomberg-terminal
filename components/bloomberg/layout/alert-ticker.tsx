...
function fmtChange(change: number | null, pct: number | null, type: string): {
...
function ItemSegment({ item }: { item: TickerItem }) {
  ...
  if (item.label === "VIX" && item.vix_level && item.vix_level !== "normal" && item.vix_level !== "low") {
    ...
    return (
      ...
      >
        ...
        {pctStr && (
          <span style={{ fontSize: 8, opacity: 0.85 }}>{pctStr}</span>
        )}
      </span>
    );
  }
  ...
  return (
    <span className="inline-flex items-baseline gap-1">
      ...
      {text && (
        <span style={{ color: changeColor, fontSize: 9 }}>{text}</span>
      )}
    </span>
  );
}
...
function AlertSegment({ alert }: { alert: TickerAlert }) {
  ...
  if (alert.type === "stoploss" && alert.current_price != null && alert.stop_price != null) {
    const distPct = ((alert.stop_price - alert.current_price) / alert.stop_price * 100).toFixed(2);
    return (
      <span
  ...
  const label = m ? `${m[1]} -> ${m[2]}` : alert.message.replace(/^[🔴🟡⚠]\s*/, "");
...
export function AlertTicker() {
  ...
  const durationSec = Math.max(30, (items.length * 4) + (alerts.length * 6));
