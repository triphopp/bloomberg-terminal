...
function getCycleZone(score: number): { label: string; color: string } {
  ...
  if (abs < 0.70) return { label: "MODERATE",   color: "#00bfff" };
...
const DRIVER_LABELS: { key: keyof NonNullable<SectorSignal["indicators"]>; label: string; weight: string; desc: string }[] = [
  { key: "z_slope",  label: "Yield Curve", weight: "30%", desc: "T10Y2Y Spread (10-Year − 2-Year Treasury Yield)\nค่า z-score เทียบช่วง 252 วันทำการที่ผ่านมา (น้ำหนัก 30%)\nบวก: yield curve ชันขึ้น (ภาวะปกติ) | ลบ: แบนตัวหรือกลับด้าน" },
  { key: "z_pmi",    label: "Activity",    weight: "30%", desc: "Chicago Fed National Activity Index (CFNAI)\nค่า z-score เทียบช่วง 36 เดือนที่ผ่านมา (น้ำหนัก 30%)\nบวก: กิจกรรมเศรษฐกิจสูงกว่าศักยภาพ | ลบ: ต่ำกว่าศักยภาพ" },
  { key: "z_spread", label: "Credit",      weight: "25%", desc: "ICE BofA US High Yield OAS (BAMLH0A0HYM2) — ค่า inverted\nค่า z-score เทียบช่วง 252 วันทำการที่ผ่านมา (น้ำหนัก 25%)\nบวก: credit spread ตึงตัว (ภาวะ risk-on) | ลบ: spread ขยายตัว (ภาวะ risk-off)" },
  { key: "z_dpmi",   label: "Momentum",    weight: "15%", desc: "CFNAI 3-Month Change — อัตราเร่งของกิจกรรมเศรษฐกิจ (น้ำหนัก 15%)\nบวก: เศรษฐกิจมีแนวโน้มเร่งตัวขึ้น | ลบ: เศรษฐกิจมีแนวโน้มชะลอตัวลง" },
];
  ...
  return (
    <div className="space-y-1 pl-2 border-l-2 border-white/5 font-mono">
      ...
      <div className="flex items-center gap-2 text-[9px] flex-wrap">
        ...
        <span className="font-bold text-[11px]" style={{ color: score >= 0 ? "#00bfff" : "#ffb300" }}>
        ...
        <Tip label={
          ...
          <span
            ...
            style={{ color: zone.color, borderColor: zone.color, border: `1px solid ${zone.color}40` }}
          >
            {zone.label}
          </span>
        </Tip>
        ...
        <span className="text-white/20 text-[8px] ml-auto">
          scale −3 ··· 0 ··· +3σ
        </span>
      </div>
        ...
        >
          ...
          <div className="flex gap-6 pt-0.5 border-t border-white/5">
            <div>
              ...
              <div className="flex gap-2 flex-wrap">
                {info.overweight.map((s: string) => (
                  <span key={s} className="text-green-400/70 font-mono">{s}</span>
                ))}
              </div>
            </div>
            <div>
              ...
              <div className="flex gap-2 flex-wrap">
                {info.underweight.map((s: string) => (
                  <span key={s} className="text-red-400/70 font-mono">{s}</span>
                ))}
              </div>
            </div>
            <div className="ml-auto text-right">
              ...
              >
                {cycleScore >= 0 ? "+" : ""}{cycleScore.toFixed(2)}
              </span>
            </div>
          </div>
        </div>
  ...
  return (
    <div className="flex flex-col gap-1 mb-2">
      {conflicts.mom_val_divergence && (
        <div className="flex items-center gap-2 px-3 py-1.5 text-[11px] border rounded
                        bg-amber-900/20 border-amber-700 text-amber-300">
          <AlertTriangle className="h-3 w-3 shrink-0" />
          <span>
            Momentum vs Valuation divergence — top MOM: {conflicts.top_momentum_sector},
            top VAL: {conflicts.top_valuation_sector}. Reduce position sizing.
          </span>
        </div>
      )}
      {conflicts.bc_mom_conflicts.length > 0 && (
        <div className="flex items-center gap-2 px-3 py-1.5 text-[11px] border rounded
          ...
          <AlertTriangle className="h-3 w-3 shrink-0" />
          <span>
            BC–MOM conflict: {conflicts.bc_mom_conflicts.join(", ")} — cycle disagrees
            with momentum. Verify thesis before trading.
          </span>
        </div>
      )}
    </div>
  ...
  return (
    <>
      <tr
        ...
        onClick={onToggle}
      >
        <td className="py-1.5 pl-3 text-[11px] font-mono text-white/40 w-8">
          {entry.rank}
        </td>
        <td className="py-1.5 text-[11px] font-mono text-white/80 w-14">
          {entry.ticker}
        </td>
        <td className="py-1.5 text-[11px] text-white/60 hidden md:table-cell">
          {entry.name}
        </td>
        <td className="py-1.5 text-[11px] font-mono w-16 text-right pr-2"
          style={{ color: entry.sector_score >= 0 ? "#22c55e" : "#ef4444" }}>
          {entry.sector_score >= 0 ? "+" : ""}{entry.sector_score.toFixed(2)}
        </td>
                ...
                return (
                  <div key={layer} className="space-y-0.5">
                    <div className="font-bold text-white/50 uppercase tracking-wider">
                      {LAYER_LABELS[layer]} ({d?.z_score != null
                    ...
                    {layer === "BC" && (
                      <>
                        ...
                        <div className="text-white/60">
                          Phase: <span className="text-white/80 font-medium">{String(d?.phase_label ?? "—")}</span>
                        </div>
                        {/* cycleScore = composite z-score that DETERMINES the phase */}
                        <div className="text-white/40 text-[9px]">
                          Mkt Cycle: {cycleScore >= 0 ? "+" : ""}{cycleScore.toFixed(2)}
                          {" "}(score ≥0 → MID_EXP, ≥1 → EARLY_REC, &lt;0 → LATE, &lt;-1 → REC)
                        </div>
                    ...
                    {layer === "MOM" && (
                      <>
                        <div className="text-white/60">
                          12M-1M: {d?.mom_12_1 != null ? `${(Number(d.mom_12_1) * 100).toFixed(1)}%` : "—"}
                        </div>
                        <div className="text-white/60">
    ...
    <div className="h-full overflow-y-auto p-3 space-y-3 text-xs">
      ...
      <div className="overflow-x-auto" style={{ borderColor: colors.border }}>
        <table className="w-full border-collapse">
          <thead>
            <tr className="border-b text-[10px] text-white/30 uppercase tracking-wider"
              style={{ borderColor: colors.border }}>
          ...
          <tbody>
            {data.sectors.map((entry) => (
              <SectorRow
                ...
                onToggle={() =>
                  setExpandedTicker(
                    expandedTicker === entry.ticker ? null : entry.ticker,
                  )
                }
                colors={colors}
