...
function buildStrategies(calls: OptionRow[], puts: OptionRow[], spot: number): Strategy[] {
  ...
  // OTM ~5%, Far OTM ~10%
  const otmCall5 = nearest(
    calls.filter((c) => c.strike > spot),
    spot * 1.05
  );
  const otmPut5 = nearest(
    puts.filter((p) => p.strike < spot),
    spot * 0.95
  );
  const otmCall10 = nearest(
    calls.filter((c) => c.strike > spot * 1.07),
    spot * 1.1
  );
  const otmPut10 = nearest(
    puts.filter((p) => p.strike < spot * 0.93),
    spot * 0.9
  );
  ...
  if (otmCall5 && otmPut5)
    (() => {
      ...
      strats.push({
        ...
        maxProfit: null,
        maxLoss: cost,
        beLow: otmPut5.strike - cost,
        beHigh: otmCall5.strike + cost,
      ...
      strats.push({
        ...
        legs: [
          { action: "SELL", kind: "CALL", strike: otmCall5.strike, premium: mid(otmCall5), qty: 1 },
          {
            action: "BUY",
            kind: "CALL",
            strike: otmCall10.strike,
            premium: mid(otmCall10),
            qty: 1,
          },
...
function buildSurfaceGrid(
  ...
  const allIVs = Object.values(grid)
...
function StatCard({
  ...
  return (
    <div
      className="flex-1 p-2 border text-center min-w-[90px]"
      style={{ borderColor: colors.border }}
    >
      <div className="text-[9px] tracking-wider mb-0.5" style={{ color: colors.textSecondary }}>
        {label}
      </div>
      <div className="text-sm font-bold font-mono" style={{ color: col ?? colors.text }}>
        {value}
      </div>
      {sub && (
...
function ChainView({
  ...
  const filtered = Array.from(new Set([...calls, ...puts].map((o) => o.strike)))
  ...
  return (
    <div className="overflow-x-auto">
        ...
        <tbody>
          {filtered.map((strike) => {
            ...
            return (
              <tr
                key={strike}
                style={{ backgroundColor: rowBg, borderBottom: `1px solid ${colors.border}` }}
              >
                {/* CALL cells */}
                <td
                  className={`${td} text-right`}
                  style={{ color: callITM ? colors.positive : colors.text }}
                >
                  {c ? fmt2(c.bid) : "—"}
                </td>
                <td
                ...
                <td className={`${td} text-right`} style={{ color: colors.textSecondary }}>
                  {c ? `${(c.impliedVolatility * 100).toFixed(1)}%` : "—"}
                </td>
                <td className={`${td} text-right`} style={{ color: colors.textSecondary }}>
                  {c ? (c.openInterest ?? 0).toLocaleString() : "—"}
                </td>
                {/* Strike */}
                <td
                ...
                <td
                  className={`${td} text-right`}
                  style={{ color: putITM ? colors.negative : colors.text }}
                >
                  {p ? fmt2(p.bid) : "—"}
                </td>
                <td
                ...
                <td className={`${td} text-right`} style={{ color: colors.textSecondary }}>
                  {p ? `${(p.impliedVolatility * 100).toFixed(1)}%` : "—"}
                </td>
                <td className={`${td} text-right`} style={{ color: colors.textSecondary }}>
                  {p ? (p.openInterest ?? 0).toLocaleString() : "—"}
                </td>
              </tr>
            );
          })}
      ...
      <p className="text-[9px] mt-2" style={{ color: colors.textSecondary }}>
        ▶ = ATM (near spot ${spot.toFixed(2)}) · Green = call ITM · Red = put ITM · Showing ±18% of
        spot
      </p>
    </div>
  );
...
function StrategiesView({
  ...
  const greeks = useMemo(
    () => (strat ? buildGreeks(strat, spot, T, ivCurrent) : []),
    [strat, spot, T, ivCurrent]
  );
  ...
  if (!strat)
    return (
      <div className="py-12 text-center text-xs" style={sec}>
        No options data to build strategies
      </div>
    );
            ...
            >
              <div className="flex items-center justify-between mb-1">
                <span
                  ...
                  style={{ color: selected === s.id ? colors.accent : colors.text }}
                >
                  {s.name}
                </span>
              ...
              <div className="flex gap-3 text-[10px]" style={sec}>
                <span>
                  ...
                  <span
                    className="font-mono"
                    style={{ color: s.netCost > 0 ? colors.negative : colors.positive }}
                  >
                    {s.netCost > 0
                      ? `Dr $${s.netCost.toFixed(2)}`
                      : `Cr $${Math.abs(s.netCost).toFixed(2)}`}
                  </span>
                </span>
                <span>
                  Max P:{" "}
        ...
        <div className="lg:col-span-2 space-y-3">
          ...
          <div className="p-3 border" style={panel}>
            <div className="flex items-center justify-between mb-2">
              <span className="text-xs font-bold tracking-wider" style={{ color: colors.accent }}>
            ...
            <table className="w-full mb-2 text-[10px]">
              ...
              <tbody>
                {strat.legs.map((l, i) => (
                  <tr
                    key={`${l.action}-${l.kind}-${l.strike}-${i}`}
                    style={{ borderTop: `1px solid ${colors.border}` }}
                  >
                    <td
                      className="py-0.5 font-bold"
                      style={{ color: l.action === "BUY" ? colors.positive : colors.negative }}
                    >
                      {l.action}
                    </td>
              ...
              {[
                {
                  ...
                  value:
                    strat.netCost > 0
                      ? `Debit $${strat.netCost.toFixed(2)}`
                      : `Credit $${Math.abs(strat.netCost).toFixed(2)}`,
                  col: strat.netCost > 0 ? colors.negative : colors.positive,
                },
                {
                  label: "Max Profit",
                  value: strat.maxProfit == null ? "Unlimited" : `$${strat.maxProfit.toFixed(2)}`,
                  col: colors.positive,
                },
                {
                  label: "Max Loss",
                  value: strat.maxLoss == null ? "Unlimited" : `$${strat.maxLoss.toFixed(2)}`,
                  col: colors.negative,
                },
                {
                  label: "Breakeven",
                  value:
                    strat.beLow != null
                      ? strat.beHigh != null
                        ? `${strat.beLow.toFixed(1)} / ${strat.beHigh.toFixed(1)}`
                        : `$${strat.beLow.toFixed(2)}`
                      : "N/A",
                  col: colors.text,
                },
              ].map(({ label, value, col }) => (
                <div
                  ...
                  style={{ borderColor: colors.border }}
                >
                  <div style={sec} className="text-[8px] tracking-wider mb-0.5">
                    {label}
                  </div>
          ...
          <div className="p-3 border" style={panel}>
            ...
            <p className="text-[9px] mb-2" style={sec}>
              {greekMode === "delta" &&
                "Δ — Rate of change of option price per $1 move in underlying"}
...
const SURFACE_METRICS: { id: SurfaceMetric; label: string; desc: string }[] = [
  { id: "iv", label: "IV%", desc: "Implied Volatility — the market's expected annual move" },
  {
    id: "call_delta",
    label: "Call Δ",
    desc: "Call Delta (0→1): how much call price moves per $1 in spot",
  },
  {
    id: "put_delta",
    label: "Put Δ",
    desc: "Put Delta (-1→0): how much put price moves per $1 in spot",
  },
  {
    id: "gamma",
    label: "Γ",
    desc: "Gamma: rate of delta change — peaks at ATM, near zero far OTM/ITM",
  },
  {
    id: "theta",
    label: "Θ/day",
    desc: "Theta: daily time decay (negative = option loses value per day)",
  },
  {
    id: "vega",
    label: "ν/1%",
    desc: "Vega: value change per +1% in IV — highest at ATM & long-dated",
  },
];
...
function computeSurfaceMetric(
  ...
  switch (metric) {
    case "call_delta":
      return +bsDelta(spot, K, T, r, sig, "call").toFixed(4);
    case "put_delta":
      return +bsDelta(spot, K, T, r, sig, "put").toFixed(4);
    case "gamma":
      return +bsGamma(spot, K, T, r, sig).toFixed(6);
    case "theta":
      return +bsTheta(spot, K, T, r, sig, "call").toFixed(4); // daily (already /365 in fn)
    case "vega":
      return +bsVega(spot, K, T, r, sig).toFixed(4);
    default:
      return 0;
  }
}
...
function fmtCellFull(v: number, metric: SurfaceMetric): string {
  switch (metric) {
    case "iv":
      return `${v.toFixed(2)}%`;
    case "call_delta":
      return `Δ = ${v.toFixed(4)}`;
    case "put_delta":
      return `Δ = ${v.toFixed(4)}`;
    case "gamma":
      return `Γ = ${v.toFixed(6)}`;
    case "theta":
      return `Θ = ${v.toFixed(4)} $/day`;
    case "vega":
      return `ν = ${v.toFixed(4)} $/1%IV`;
  }
}
...
function SurfaceView({
  ...
  const smileCalls = calls
    ...
    .map((c) => ({
      strike: c.strike,
      callIV: +(c.impliedVolatility * 100).toFixed(2),
      label: `$${c.strike}`,
    }));
  ...
  const { cellValues, minV, maxV } = useMemo(() => {
    ...
    return {
      cellValues: vals,
      minV: Number.isFinite(lo) ? lo : 0,
      maxV: Number.isFinite(hi) ? hi : 1,
    };
  }, [grid, usedExpiries, metric, spot]);

  // metric is a SurfaceMetric, so this always matches one of SURFACE_METRICS —
  // the fallback to [0] is just to avoid a non-null assertion, never hit in practice.
  const currentMetricInfo = SURFACE_METRICS.find((m) => m.id === metric) ?? SURFACE_METRICS[0];
  const panel = { backgroundColor: colors.surface, borderColor: colors.border };
  const sec = { color: colors.textSecondary };
    ...
    <div className="space-y-4">
      ...
      <div className="p-4 border" style={panel}>
        ...
        {smileMerged.length === 0 ? (
          <div className="py-8 text-center text-xs" style={sec}>
            No IV data for current expiry
          </div>
        ) : (
          <>
            ...
            <p className="text-[9px] mt-1" style={sec}>
              Green = Calls · Red = Puts · <span style={{ color: colors.text }}>Skew</span> (puts
              pricier than calls) = market pricing in downside risk
            </p>
          </>
        )}
      </div>
      ...
      <div className="p-4 border" style={panel}>
        ...
        {surfaceEnabled && surfaceQuery.isSuccess && usedExpiries.length > 0 && (
          <div className="overflow-x-auto">
            ...
            <div className="flex items-center gap-2 mb-3 text-[9px]" style={sec}>
              ...
              <div className="flex h-3 rounded overflow-hidden" style={{ width: 160 }}>
                {Array.from({ length: 24 }, (_, i) => {
                  ...
                  return (
                    <div
                      // biome-ignore lint/suspicious/noArrayIndexKey: fixed-length decorative gradient legend, never reordered or mutated
                      key={i}
                      className="flex-1 h-3"
                      style={{ backgroundColor: metricColor(v, metric, minV, maxV) }}
                    />
                  );
                })}
              </div>
            ...
            <table className="text-[9px] border-collapse">
              ...
              <tbody>
                {usedExpiries.map((exp) => (
                  <tr key={exp}>
                    <td className="text-right pr-2 py-0.5 font-mono whitespace-nowrap" style={sec}>
                      {exp}
                    </td>
                    {MONO_BUCKETS.map((b) => {
                      ...
                      const tooltip =
                        rawIV > 0 ? `${fmtCellFull(v, metric)} | IV=${rawIV.toFixed(1)}%` : "—";
                      return (
                        <td
                          key={b}
            ...
            <p className="text-[9px] mt-2" style={sec}>
              X = moneyness from ATM (0%) · Y = expiration · Color intensity ={" "}
              {currentMetricInfo.label} magnitude · Greeks computed with Black-Scholes (r=5%, IV
              from market)
            </p>
          </div>
        )}
        ...
        {surfaceEnabled && surfaceQuery.isSuccess && usedExpiries.length === 0 && (
          <p className="text-xs py-6 text-center" style={sec}>
            No surface data available for {symbol}
          </p>
        )}
        ...
        {!surfaceEnabled && (
          <div className="py-8 text-center space-y-2">
            <p className="text-xs" style={sec}>
              Surface requires fetching all expirations (~10 API calls). Click <strong>LOAD</strong>{" "}
              to proceed.
            </p>
            <p className="text-[9px]" style={sec}>
              Once loaded, switch between IV / Δ / Γ / Θ / ν to see each Greek's shape across the
              surface
            </p>
          </div>
        )}
  ...
  return (
    <div className="space-y-3">
      <div className="flex flex-wrap gap-2 items-center">
        <span className="text-[9px]" style={sec}>
          Strategy:
        </span>
        <select
          value={gs}
        ...
        <span className="text-[9px] ml-2" style={sec}>
          Greek:
        </span>
        <div className="flex gap-1">
          {(["delta", "gamma", "theta", "vega"] as const).map((g) => (
            <button
              ...
              className="px-2 py-0.5 text-[10px] border"
              style={{
                borderColor: gm === g ? colors.accent : colors.border,
                backgroundColor: gm === g ? colors.accent : "transparent",
                color: gm === g ? "#000" : colors.text,
              }}
            >
              {g.toUpperCase()}
            </button>
  ...
  if (query.isLoading)
    return (
      <div className="flex items-center justify-center py-24">
        ...
        <span className="text-sm" style={sec}>
          Loading options data for {symbol}…
        </span>
      </div>
    );

  if (query.isError || data?.error)
    return (
      <div className="py-12 text-center space-y-2">
        ...
        <p className="text-sm" style={{ color: colors.negative }}>
          Options data unavailable for {symbol}
        </p>
        <p className="text-xs" style={sec}>
          {data?.error ?? "Options may not trade on this security"}
        </p>
      </div>
    );
    ...
    <div className="space-y-3">
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <StatCard
          ...
          sub={data.pcRatio > 1 ? "Put-heavy" : "Call-heavy"}
          col={data.pcRatio > 1 ? colors.negative : colors.positive}
          colors={colors}
        />
        <StatCard
          label="CALL OI"
          value={data.callOI.toLocaleString()}
          sub="open interest"
          col={colors.positive}
          colors={colors}
        />
        <StatCard
          label="PUT OI"
          value={data.putOI.toLocaleString()}
          sub="open interest"
          col={colors.negative}
          colors={colors}
        />
        <StatCard
          label="CALL VOL"
          value={(data.callVolume ?? 0).toLocaleString()}
          sub="volume today"
          colors={colors}
        />
        <StatCard
          label="PUT VOL"
          value={(data.putVolume ?? 0).toLocaleString()}
          sub="volume today"
          colors={colors}
        />
      </div>
      ...
      <div className="flex items-center gap-1 flex-wrap">
        <span className="text-[9px] font-bold tracking-wider mr-1" style={sec}>
          EXPIRY:
        </span>
        ...
        {data.expirations.length > 12 && (
          <span className="text-[9px]" style={sec}>
            +{data.expirations.length - 12} more
          </span>
        )}
      </div>
