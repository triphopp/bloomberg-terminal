...
type HeatGroup = "global" | "sectors" | "commodities" | "bonds"
  | "sectors_th" | "sectors_cn" | "sectors_kr" | "sectors_eu";
...
type SectorHolding = {
  ...
  price: number; pctChange: number;
  pe: number | null; forwardPE: number | null; peg: number | null;
  debtEq: number | null; marketCap: number | null;
};
...
type SectorStock = {
  symbol: string; id: string; name: string; price: number;
  pctChange: number; ytd: number; size: number;
};

// Extended stock data from intl-sectors endpoint (includes fundamentals)
type IntlStockDetail = SectorStock & {
  pe?: number | null; forwardPE?: number | null; peg?: number | null;
  debtEq?: number | null; marketCap?: number | null; weight?: number;
};
...
const ETF_TO_SECTOR: Record<string, string> = {
  ...
  XLP: "Consumer Stpls", XLB: "Materials", XLU: "Utilities",
...
function buildSectorNestedTree(tiles: HeatTile[], stocks: Record<string, SectorStock[]>): TreeNode[] {
  return tiles
    .map(t => {
      const children = stocks[t.id] ?? [];
      if (children.length === 0) {
        return {
          name: t.id, symbol: t.symbol, size: t.size ?? 1,
          pctChange: t.pctChange ?? 0, ytd: t.ytd ?? 0, price: t.value,
        };
      }
      return {
        name: t.id,
...
function TreeCell(props: any) {
  const {
    ...
    name = "", pctChange = 0, ytd = 0, price, symbol,
    children: nodeChildren,
    isDark, showYTD, onCellClick,
  } = props;
  ...
  if (Array.isArray(nodeChildren) && nodeChildren.length > 0) {
    ...
    return (
      <g>
        <rect x={x} y={y} width={width} height={height} fill={groupBg} stroke={groupBorder} strokeWidth={2} />
        {width > 40 && (
          <text x={x + 6} y={y + 14} fill={labelColor} fontSize={9} fontWeight="bold" fontFamily="monospace">
            {name}
          </text>
        )}
      </g>
  ...
  let   rowY     = y + height / 2 - blockH / 2 + lineH * 0.8;

  const rows: { text: string; bold: boolean; size: number }[] = [];
  if (showLbl) rows.push({ text: name.length > 13 ? `${name.slice(0, 12)}…` : name, bold: true,  size: Math.min(fSize, 12) });
  if (showVal) rows.push({ text: fmtTreePrice(price as number),                       bold: false, size: Math.min(fSize * 0.9, 11) });
  if (showPct) rows.push({ text: `${displayPct >= 0 ? "+" : ""}${displayPct.toFixed(2)}%`, bold: false, size: Math.min(fSize * 0.85, 10) });

  return (
    <g style={{ cursor: onCellClick ? "pointer" : "default" }} onClick={onCellClick}>
      <rect x={x + 1} y={y + 1} width={width - 2} height={height - 2} fill={cs.bg} stroke={cs.border} strokeWidth={1} rx={1} />
      {rows.map((row, i) => (
        <text
          key={i}
          x={x + width / 2}
...
function TreemapSection({
  activeGroup, mktData, currentTiles, sectorStocks, intlStocks, isDark, showYTD, onNavigate,
}: {
  ...
  const hasSectorStocks = (activeGroup === "sectors" && Object.keys(sectorStocks).length > 0)
    || (isIntl && Object.keys(intlStocks).length > 0);

  const treeData = useMemo<TreeNode[]>(() => {
    if (activeGroup === "global") return buildGlobalTreemap(mktData);
    if (isIntl && Object.keys(intlStocks).length > 0) return buildSectorNestedTree(currentTiles, intlStocks);
    if (activeGroup === "sectors" && Object.keys(sectorStocks).length > 0) return buildSectorNestedTree(currentTiles, sectorStocks);
  ...
  const renderCell = (props: any) => {
    ...
    const sym = activeGroup !== "global" ? props.symbol as string | undefined : undefined;
...
function TabBtn({
  active, onClick, label, accent, text, border,
}: { active: boolean; onClick: () => void; label: string; accent: string; text: string; border: string }) {
  return (
    <button
      type="button"
      onClick={onClick}
...
function ColumnSection({
  label, rows, isDark, isLoading, showYTD, onNavigate,
}: {
  label:     string;
  rows:      ColRow[];
  isDark:    boolean;
  ...
  const maxGainer  = items.length > 0 ? items.reduce((a, b) => b.pctChange > a.pctChange ? b : a, items[0]) : null;
  const maxLoser   = items.length > 0 ? items.reduce((a, b) => b.pctChange < a.pctChange ? b : a, items[0]) : null;

  return (
    <div className="flex flex-col h-full overflow-hidden" style={{ borderRight: "1px solid #1a1a1a" }}>
      ...
      >
        <span className="text-[9px] font-bold font-mono tracking-widest" style={{ color: accentColor }}>
          {label}
        </span>
      </div>
      ...
      {items.length > 0 && !isLoading && (
        ...
        >
          ...
          <span>
            avg <span style={{ color: avgPct >= 0 ? "#00FF00" : "#FF0000" }}>
              {avgPct >= 0 ? "+" : ""}{avgPct.toFixed(2)}%
            </span>
          </span>
          {maxGainer && maxGainer.pctChange > 0 && (
            <span>
              best <span style={{ color: "#00FF00" }}>{maxGainer.id} +{maxGainer.pctChange.toFixed(2)}%</span>
            </span>
          )}
          {maxLoser && maxLoser.pctChange < 0 && (
            <span>
              worst <span style={{ color: "#FF0000" }}>{maxLoser.id} {maxLoser.pctChange.toFixed(2)}%</span>
            </span>
          )}
        </div>
      )}
          ...
          rows.map((row, idx) => {
            ...
            return (
              ...
              >
                ...
                >
                  {row.change >= 0 ? "+" : ""}{fmtTreePrice(Math.abs(row.change) < 1 ? parseFloat(row.change.toFixed(4)) : parseFloat(row.change.toFixed(2)))}
                </span>
                <span
                  className="w-[56px] text-right text-[9px] font-bold font-mono leading-none"
                  style={{ color: activeColor }}
                >
                  {activeVal >= 0 ? "+" : ""}{activeVal.toFixed(2)}%
                </span>
              </div>
            );
          })
...
function SectorColumn({
  tiles, isDark, isLoading, showYTD, onNavigate,
  activeTab, onTabChange, intlTiles, intlStocks, intlLoading,
  intlDetail, intlAggregates,
}: {
  tiles:      HeatTile[];
  isDark:     boolean;
  isLoading:  boolean;
  ...
  const handleClick = (sectorId: string, etfSymbol: string) => {
    if (expandedSymbol === etfSymbol) { setExpandedSymbol(null); return; }
    ...
    fetch(`/api/heatmap/sector-detail?etf=${etfSymbol}&limit=10`)
      ...
      .catch(() => setDetail(p => ({ ...p, [etfSymbol]: { etf: etfSymbol, sector: sectorId, holdings: [], aggregate: { count: 0, pe: { median: null, avg: null, wtd: null, min: null, max: null, n: 0 }, forwardPE: { median: null, avg: null, wtd: null, min: null, max: null, n: 0 }, peg: { median: null, avg: null, min: null, max: null, n: 0 }, debtEq: { median: null, avg: null, min: null, max: null, n: 0 }, marketCap: { median: null, avg: null, min: null, max: null, total: null, n: 0 } } } })))
    ...
    <div className="flex flex-col h-full overflow-hidden" style={{ borderRight: "1px solid #1a1a1a" }}>
      ...
      >
        {(["us", "th", "cn", "kr", "eu"] as const).map(tab => {
          const labels: Record<string, string> = { us: "US", th: "🇹🇭TH", cn: "🇨🇳CN", kr: "🇰🇷KR", eu: "🇪🇺EU" };
      ...
      {(() => {
        ...
        return (
          ...
          >
            ...
            <span>avg <span style={{ color: avg >= 0 ? "#00FF00" : "#FF0000" }}>{avg >= 0 ? "+" : ""}{avg.toFixed(2)}%</span></span>
          </div>
        );
      })()}
      ...
      >
        ...
        {activeTab !== "us" && (intlLoading ? (
            ...
            return (
              <div key={`intl-${sectorTile.id}-${si}`}>
                ...
                >
                  ...
                  <span className="flex-1 text-[10px] font-bold font-mono truncate leading-none" style={{ color: accentColor }}>
                    {sectorTile.id}
                  </span>
                  <span className="w-[20px] text-right text-[7px] font-mono" style={{ color: "#555" }}>
                    {stks.length}
                  </span>
                  <span className="w-[64px] text-right text-[9px] font-bold font-mono leading-none" style={{ color: activeColor }}>
                    {activeVal >= 0 ? "+" : ""}{activeVal.toFixed(2)}%
                  </span>
                </button>
                {isSectorExpanded && (
                      ...
                      return (
                        ...
                        >
                          <div className="flex flex-wrap gap-x-3 gap-y-0.5">
                            <span>
                              P/E <span style={{ color: accentColor }}>{fmtMetric(agg.pe.median)}</span>
                              <span style={{ color: "#555" }}> med</span>
                              {agg.pe.wtd != null && (
                                <span> | <span style={{ color: "#aaa" }}>{fmtMetric(agg.pe.wtd)}</span><span style={{ color: "#555" }}> wtd</span></span>
                              )}
                              <span style={{ color: "#444" }}> [{fmtMetric(agg.pe.min)}–{fmtMetric(agg.pe.max)}]</span>
                            </span>
                            <span>
                              Fwd P/E <span style={{ color: accentColor }}>{fmtMetric(agg.forwardPE.median)}</span>
                              <span style={{ color: "#555" }}> med</span>
                              {agg.forwardPE.wtd != null && (
                                <span> | <span style={{ color: "#aaa" }}>{fmtMetric(agg.forwardPE.wtd)}</span><span style={{ color: "#555" }}> wtd</span></span>
                              )}
                            </span>
                          </div>
                          <div className="flex flex-wrap gap-x-3 gap-y-0.5 mt-0.5">
                            <span>
                              PEG <span style={{ color: accentColor }}>{fmtMetric(agg.peg.median)}</span>
                              <span style={{ color: "#444" }}> [{fmtMetric(agg.peg.min)}–{fmtMetric(agg.peg.max)}]</span>
                            </span>
                            <span>
                              D/E <span style={{ color: accentColor }}>{fmtMetric(agg.debtEq.median)}%</span>
                              <span style={{ color: "#444" }}> [{fmtMetric(agg.debtEq.min)}–{fmtMetric(agg.debtEq.max)}%]</span>
                            </span>
                            <span>
                              MCap <span style={{ color: accentColor }}>{fmtMCap(agg.marketCap.median)}</span>
                              <span style={{ color: "#555" }}> med</span>
                              {agg.marketCap.total != null && (
                                <span> | Σ <span style={{ color: "#aaa" }}>{fmtMCap(agg.marketCap.total)}</span></span>
                              )}
                            </span>
                    ...
                    {(intlDetail[sectorTile.id] ?? stks).map(s => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <span className="flex-1 text-[9px] font-bold font-mono truncate leading-none" style={{ color: accentColor }}>
                            {s.id}
                          </span>
                          {det.weight != null && det.weight > 0 && (
                            <span className="w-[36px] text-right text-[7px] font-mono leading-none" style={{ color: "#555" }}>
                              {det.weight.toFixed(1)}%
                            </span>
                          )}
                          <span className="w-[60px] text-right text-[8px] font-mono leading-none" style={{ color: valColor }}>
                            {fmtTreePrice(s.price)}
                          </span>
                          <span className="w-[50px] text-right text-[8px] font-bold font-mono leading-none" style={{ color: sChgColor }}>
                            {sActiveVal >= 0 ? "+" : ""}{sActiveVal.toFixed(2)}%
                          </span>
                          <span className="w-[44px] text-right text-[7px] font-mono leading-none" style={{ color: "#777" }}>
          ...
          <div className="flex items-center justify-center pt-8 text-[9px] font-mono" style={{ color: "#555" }}>
                  ...
                  <div style={{ background: "#080808", borderBottom: "1px solid #1a1a1a" }}>
                    {loading ? (
                      <div className="flex justify-center items-center py-3">
                        <RefreshCw className="h-3 w-3 animate-spin" style={{ color: accentColor }} />
                      </div>
                      ...
                      <>
                        ...
                        >
                          <div className="flex flex-wrap gap-x-3 gap-y-0.5">
                            <span>
                              P/E <span style={{ color: accentColor }}>{fmtMetric(d.aggregate.pe.median)}</span>
                              <span style={{ color: "#555" }}> med</span>
                              {d.aggregate.pe.wtd != null && (
                                <span> | <span style={{ color: "#aaa" }}>{fmtMetric(d.aggregate.pe.wtd)}</span><span style={{ color: "#555" }}> wtd</span></span>
                              )}
                              <span style={{ color: "#444" }}> [{fmtMetric(d.aggregate.pe.min)}–{fmtMetric(d.aggregate.pe.max)}]</span>
                            </span>
                            <span>
                              Fwd P/E <span style={{ color: accentColor }}>{fmtMetric(d.aggregate.forwardPE.median)}</span>
                              <span style={{ color: "#555" }}> med</span>
                              {d.aggregate.forwardPE.wtd != null && (
                                <span> | <span style={{ color: "#aaa" }}>{fmtMetric(d.aggregate.forwardPE.wtd)}</span><span style={{ color: "#555" }}> wtd</span></span>
                              )}
                            </span>
                          </div>
                          <div className="flex flex-wrap gap-x-3 gap-y-0.5 mt-0.5">
                            <span>
                              PEG <span style={{ color: accentColor }}>{fmtMetric(d.aggregate.peg.median)}</span>
                              <span style={{ color: "#444" }}> [{fmtMetric(d.aggregate.peg.min)}–{fmtMetric(d.aggregate.peg.max)}]</span>
                            </span>
                            <span>
                              D/E <span style={{ color: accentColor }}>{fmtMetric(d.aggregate.debtEq.median)}%</span>
                              <span style={{ color: "#444" }}> [{fmtMetric(d.aggregate.debtEq.min)}–{fmtMetric(d.aggregate.debtEq.max)}%]</span>
                            </span>
                            <span>
                              MCap <span style={{ color: accentColor }}>{fmtMCap(d.aggregate.marketCap.median)}</span>
                              <span style={{ color: "#555" }}> med</span>
                              {d.aggregate.marketCap.total != null && (
                                <span> | Σ <span style={{ color: "#aaa" }}>{fmtMCap(d.aggregate.marketCap.total)}</span></span>
                              )}
                            </span>
                        ...
                        {d.holdings.map(h => {
                          ...
                          return (
                            ...
                            >
                              <span className="w-[10px] text-[7px] font-mono" style={{ color: "#555" }}>
                                {d.holdings.indexOf(h) + 1}
                              </span>
                              <span className="w-[48px] text-[9px] font-bold font-mono truncate leading-none" style={{ color: accentColor }}>
                                {h.symbol}
                              </span>
                              <span className="w-[42px] text-right text-[8px] font-mono leading-none" style={{ color: "#666" }}>
                                {h.weight.toFixed(1)}%
                              </span>
                              <span className="flex-1 text-right text-[8px] font-mono leading-none" style={{ color: valColor }}>
                                {h.price.toFixed(2)}
                              </span>
                              <span className="w-[52px] text-right text-[8px] font-bold font-mono leading-none" style={{ color: hChgColor }}>
...
interface MarketMoversViewProps {
  ...
  onBack:     () => void;
  marketData: {
    americas:    { id: string; value: number; change: number; pctChange: number; ytd: number; size?: number }[];
    emea:        { id: string; value: number; change: number; pctChange: number; ytd: number; size?: number }[];
    asiaPacific: { id: string; value: number; change: number; pctChange: number; ytd: number; size?: number }[];
    lastUpdated?: string;
  };
  onRefresh:  () => void;
  isLoading:  boolean;
...
export default function MarketMoversView({
  ...
  const [intlData, setIntlData] = useState<Record<string, {
    tiles: HeatTile[];
  ...
  useEffect(() => {
    ...
    load("sectors"); load("commodities"); load("bonds");
  ...
  const fetchIntlCountry = (country: IntlCountry) => {
    ...
    fetch(`/api/heatmap/intl-sectors?country=${country}`)
      .then(r => r.json())
      .then(d => {
        const tiles: HeatTile[] = (d.tiles ?? []).map((t: any) => ({
          id: t.id, symbol: t.symbol, value: t.value ?? 0,
          change: 0, pctChange: t.pctChange ?? 0, ytd: t.ytd ?? 0,
          avat: 0, time: "", size: t.size ?? 1,
        }));
        ...
        for (const [sector, stks] of Object.entries(d.stocks ?? {})) {
          const parsed = (stks as any[]).map(s => ({
            symbol: s.symbol, id: s.id, name: s.name ?? s.id,
            price: s.price, pctChange: s.pctChange, ytd: s.ytd ?? 0, size: s.size ?? 1,
            pe: s.pe ?? null, forwardPE: s.forwardPE ?? null, peg: s.peg ?? null,
            debtEq: s.debtEq ?? null, marketCap: s.marketCap ?? null, weight: s.weight ?? 0,
          }));
      ...
      .catch(() => setIntlData(p => ({ ...p, [country]: { tiles: [], stocks: {}, detail: {}, aggregates: {} } })))
  ...
  }, [tableSectorTab]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const currentTiles   = intlCountry
    ...
    : activeGroup !== "global" ? (groupTiles[activeGroup] ?? []) : [];
  const isGroupLoading = intlCountry
    ? !!loadingIntl[intlCountry]
    : !!loadingGroups[activeGroup];
  ...
  const globalRows = useMemo((): ColRow[] => {
    ...
    for (const [key, label] of regions) {
      const items = (marketData[key] as { id: string; value: number; change: number; pctChange: number; ytd: number }[]) ?? [];
      if (items.length === 0) continue;
      result.push({ kind: "subheader", label });
      for (const item of items) {
        result.push({ kind: "item", id: item.id, value: item.value, change: item.change ?? 0, pctChange: item.pctChange, ytd: item.ytd });
      }
    }
    return result;
  }, [marketData]);
  ...
  const tileToRows = (tiles: HeatTile[]): ColRow[] =>
    tiles.map(t => ({ kind: "item" as const, id: t.id, value: t.value, change: t.change ?? 0, pctChange: t.pctChange, ytd: t.ytd }));

  // Merged COMMODITIES + BONDS rows
  const commoditiesBondsRows = useMemo((): ColRow[] => {
    ...
    return result;
  }, [groupTiles.commodities, groupTiles.bonds]); // eslint-disable-line react-hooks/exhaustive-deps
      ...
      {viewMode === "table" && (
        <div className="flex-1 grid grid-cols-3 overflow-hidden min-h-0">
          ...
          <SectorColumn
            ...
            intlAggregates={tableSectorTab !== "us" ? (intlData[tableSectorTab]?.aggregates ?? {}) : {}}
          />
          <ColumnSection
            label="COMMODITIES & BONDS"
            rows={commoditiesBondsRows}
            isDark={isDarkMode}
            isLoading={(!!loadingGroups.commodities && !groupTiles.commodities) || (!!loadingGroups.bonds && !groupTiles.bonds)}
            showYTD={showYTD}
          />
        </div>
      )}
      ...
      {viewMode === "heatmap" && (
        <div className="flex-1 overflow-y-auto min-h-0">
          ...
          <div className="p-1">
            ...
            {activeGroup !== "global" && !isGroupLoading && !intlCountry && currentTiles.length === 0 && (
              <div className="flex items-center justify-center py-20">
