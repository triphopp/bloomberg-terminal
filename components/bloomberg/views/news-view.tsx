...
type Colors = { accent: string; text: string; textSecondary: string; border: string; surface: string; background: string };
  ...
  return (
    <div className="flex items-center gap-1.5">
      <div className="flex-1 h-1 rounded-sm" style={{ backgroundColor: "#1a1a1a" }}>
        <div className="h-full rounded-sm transition-all" style={{ width: `${Math.min(100, pct)}%`, backgroundColor: barClr }} />
      </div>
      <span className="text-[9px] font-bold font-mono w-8 text-right shrink-0" style={{ color: barClr }}>{pct}%</span>
      <span className="text-[8px] font-mono shrink-0" style={{ color: colors.textSecondary }}>YES</span>
    </div>
  );
}
...
function PolymarketColumn({ colors, isDark }: { colors: Colors; isDark: boolean }) {
  ...
  const asOfStr = asOf ? new Date(asOf).toLocaleTimeString("en-US", { hour: "2-digit", minute: "2-digit" }) : "";
  ...
  return (
    ...
    >
      ...
      >
        <span className="text-[9px] font-bold font-mono tracking-widest" style={{ color: colors.accent }}>
          ⚡ POLYMARKET
        </span>
        <div className="flex items-center gap-2">
          {asOfStr && !isSearching && (
            <span className="text-[8px] font-mono" style={{ color: colors.textSecondary }}>{asOfStr}</span>
          )}
          {!isSearching && (
            <button type="button" onClick={refresh} disabled={loading} className="hover:opacity-70" title="Refresh">
              <RefreshCw className={`h-2.5 w-2.5 ${loading ? "animate-spin" : ""}`} style={{ color: colors.accent }} />
            </button>
          )}
        </div>
      </div>
      ...
      {/* Search bar */}
      <div className="shrink-0 flex items-center gap-1 px-2 py-1 border-b" style={{ borderColor: colors.border }}>
        <input
          type="text"
          value={query}
        ...
        />
        {query && (
          <button type="button" onClick={() => { setQuery(""); setSearchResults([]); }} className="hover:opacity-70 shrink-0">
            <X className="h-2.5 w-2.5" style={{ color: colors.textSecondary }} />
          </button>
        )}
        {searchLoading && <RefreshCw className="h-2.5 w-2.5 animate-spin shrink-0" style={{ color: colors.accent }} />}
      </div>
        ...
        {isSearching && (
          <>
            {!searchLoading && searchResults.length === 0 && (
              <div className="py-8 text-center text-[9px] font-mono" style={{ color: colors.textSecondary }}>no results</div>
            )}
            {searchResults.map(m => (
              <a
                key={m.slug}
              ...
              >
                <p className="text-[9px] font-mono leading-snug mb-1.5 group-hover:underline line-clamp-2" style={{ color: colors.text }}>
                  {m.question}
                </p>
                ...
                <div className="flex items-center justify-between mt-1">
                  <span className="text-[8px] font-mono" style={{ color: colors.textSecondary }}>
                    {fmtVol(m.volume)}{m.end_date ? ` · ${fmtEndDate(m.end_date)}` : ""}
                  </span>
                  <ExternalLink className="h-2 w-2 opacity-0 group-hover:opacity-40" style={{ color: colors.textSecondary }} />
                </div>
              </a>
            ))}
          </>
        ...
        {!isSearching && (
          <>
            {loading && signals.length === 0 && (
              <div className="flex items-center justify-center py-10 gap-2" style={{ color: colors.textSecondary }}>
            ...
            {signals.map(sig => {
              ...
              const dirColor = sig.direction === "UP" ? "#4caf50" : sig.direction === "DOWN" ? "#ef5350" : "#616161";
              const statusColor = sig.status === "LIKELY" ? "#4caf50" : sig.status === "UNLIKELY" ? "#ef5350" : "#ffc107";
              const deltaStr = sig.delta_24h != null
                ...
                >
                  ...
                  <div className="flex items-center justify-between mb-1">
                    <span
                      ...
                      style={{ color: sig.color, backgroundColor: `${sig.color}18`, border: `1px solid ${sig.color}44` }}
                    >
                      {sig.label.toUpperCase()}
                    </span>
                    <span className="text-[8px] font-mono" style={{ color: colors.textSecondary }}>{fmtVol(sig.volume)}</span>
                  </div>
                  ...
                  <div className="flex items-center gap-2 mt-1 mb-0.5">
                    ...
                    {sig.regime_flag === "HIGH_CONVICTION" && (
                      <span className="text-[7px] font-mono px-1" style={{ color: "#ff9800", border: "1px solid #ff980044" }}>
                        CONV
                      </span>
                    )}
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <ExternalLink className="h-2 w-2 shrink-0 opacity-0 group-hover:opacity-40 ml-auto" style={{ color: colors.textSecondary }} />
                  </div>
                </a>
              );
            })}
...
export default function NewsView({ isDarkMode, onBack }: NewsViewProps) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (saved) {
        ...
        const valid = parsed.filter(h => (PLATFORMS as string[]).includes(h.platform)) as SocialHandle[];
  ...
  const fetchFeed = useCallback(async (topicList: string[]) => {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/news/feed?topics=${encodeURIComponent(topicList.join(","))}&limit=80`);
  ...
  const addTopic = () => {
    ...
    if (!t || topics.map(x => x.toUpperCase()).includes(t)) { setNewTopic(""); return; }
  ...
  const handleAddHandle = () => {
    ...
    if (already) { setNewHandle(""); return; }
  ...
  const handleRemoveHandle = (platform: Platform, handle: string) => {
    const updated = socialHandles.filter(
      h => !(h.platform === platform && h.handle === handle),
    );
    setSocialHandles(updated);
    localStorage.setItem(SOCIAL_KEY, JSON.stringify(updated));
  ...
  // ── Render ───────────────────────────────────────────────────────────────────
  return (
    <div className="h-full flex flex-col font-mono overflow-hidden" style={{ backgroundColor: colors.background, color: colors.text }}>

      {/* ── Top bar ─────────────────────────────────────────────────────────── */}
      <div className="shrink-0 flex items-center gap-2 px-2 py-1 border-b" style={{ backgroundColor: colors.surface, borderColor: colors.border }}>
        {/* Tabs */}
      ...
      {activeTab === "feed" && (
        <div className="flex flex-col flex-1 min-h-0 min-w-0">
          ...
          {/* Topic chips bar */}
          <div className="shrink-0 flex flex-wrap items-center gap-1.5 px-2 py-1.5 border-b" style={{ borderColor: colors.border }}>
            {topics.map(t => (
              <span
                key={t}
            ...
            {articles.length > 0 && (
              <span className="ml-auto text-xs" style={{ color: colors.textSecondary }}>
                {filterText ? `${filtered.length} / ` : ""}{articles.length} articles
              </span>
            )}
          </div>
          ...
          <div className="flex-1 overflow-y-auto" style={{ scrollbarWidth: "thin", scrollbarColor: "#333 #000" }}>
            {error && (
              <div className="m-2 p-2 text-xs border" style={{ borderColor: colors.negative, color: colors.negative }}>
                {error}
              </div>
            )}
            ...
            {loading ? (
              <div className="flex items-center justify-center py-16">
                ...
                <span className="text-xs" style={{ color: colors.textSecondary }}>Fetching news…</span>
              </div>
            ) : filtered.length === 0 ? (
              ...
              <div className="divide-y" style={{ borderColor: colors.border }}>
                {filtered.map((article, i) => (
                  ...
                  >
                    ...
                    {/* Time */}
                    <span className="shrink-0 mt-0.5 text-[10px] w-8 text-right" style={{ color: colors.textSecondary }}>
                      {timeAgo(article.published_at)}
                    </span>

                    {/* Headline */}
                    <span className="flex-1 text-[10px] leading-snug group-hover:underline" style={{ color: colors.text }}>
                      {article.title}
                    </span>

                    {/* Link icon */}
                    <ExternalLink className="shrink-0 h-2.5 w-2.5 mt-0.5 opacity-0 group-hover:opacity-60" style={{ color: colors.textSecondary }} />
                  </a>
                ))}
              </div>
            )}
      ...
      {activeTab === "social" && (
        <div className="flex flex-col flex-1 min-h-0 min-w-0">
          ...
          {/* Add handle bar */}
          <div className="shrink-0 flex items-center gap-1.5 px-2 py-1.5 border-b" style={{ borderColor: colors.border }}>
            {/* Platform selector */}
            <select
            ...
            >
              {PLATFORMS.map(p => (
                <option key={p} value={p} style={{ color: PLATFORM_INFO[p].color, backgroundColor: colors.surface }}>
            ...
            {/* Handle input */}
            <form onSubmit={e => { e.preventDefault(); handleAddHandle(); }} className="flex-1 flex items-center gap-1">
              <input
                type="text"
                value={newHandle}
            ...
            {socialHandles.length > 0 && (
              <BloombergButton color="default" onClick={() => { setSocialPosts([]); fetchSocialPosts(socialHandles); }} disabled={socialLoading}>
          ...
          {socialHandles.length > 0 && (
            <div className="shrink-0 flex flex-wrap gap-1.5 px-2 py-1.5 border-b" style={{ borderColor: colors.border }}>
          ...
          <div className="flex-1 overflow-y-auto" style={{ scrollbarWidth: "thin", scrollbarColor: "#333 #000" }}>
            ...
            {/* Error */}
            {socialError && (
              <div className="m-2 p-2 text-xs border" style={{ borderColor: colors.negative, color: colors.negative, backgroundColor: `${colors.negative}10` }}>
                {socialError}
              </div>
            )}
            ...
            {socialHandles.length === 0 && (
              <div className="py-16 text-center space-y-3">
                <p className="text-[10px] font-bold" style={{ color: colors.textSecondary }}>No sources added yet</p>
            ...
            {socialLoading && (
              <div className="flex items-center justify-center py-16">
                <RefreshCw className="h-4 w-4 animate-spin mr-2" style={{ color: colors.accent }} />
                <span className="text-xs" style={{ color: colors.textSecondary }}>Fetching social feed…</span>
              </div>
            )}
                ...
                {socialPosts.map((post, i) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {/* Source name + time */}
                      <div className="shrink-0 mt-0.5 text-[10px] text-right w-20 leading-tight" style={{ color: colors.textSecondary }}>
