...
import { useStockQuote, useStockHistory, useStockSearch } from "../hooks/useStockData";
import { PinnedAssets } from "./pinned-assets";
import { SectorRegimeHeatmap } from "./sector-regime-heatmap";
import type { MarketItem } from "../types";
import { ModularChart, IndicatorPicker, useChartIndicators, useChartTimeframe, TIME_PERIODS, BAR_INTERVALS, PERIOD_LABEL, INTERVAL_LABEL, INTERVAL_VALID_RANGES } from "../chart";
import { FearGreedPane } from "../chart/FearGreedPane";
import type { OhlcvBar, IndicatorRegistryEntry, TimePeriod, BarInterval } from "../chart";
import {
  Activity,
  BarChart2,
  ChevronDown,
...
function saveLayout(layout: LayoutSettings) {
  try { localStorage.setItem(LS_LAYOUT_KEY, JSON.stringify(layout)); } catch { /* ignore */ }
}
...
function TickRow({ item, colors, isSelected, onClick }: {
  item: MarketItem; colors: typeof bloombergColors.dark; isSelected: boolean; onClick: () => void;
}) {
  const isUp = item.pctChange >= 0;
  const pctColor = isUp ? "#00FF00" : "#FF0000";
  return (
    <tr className="cursor-pointer hover:bg-[#111] transition-colors"
      style={{
      ...
      <td className="px-1 py-0.5 text-left">
        <span className="font-bold text-[10px]" style={{ color: isSelected ? "#00FFFF" : colors.accent }}>{item.id}</span>
      </td>
      <td className="px-1 py-0.5 text-right font-bold text-[10px]" style={{ color: colors.text }}>{fmtPrice(item.value)}</td>
      ...
      <td className="px-1 py-0.5 text-right text-[9px]">
        {item.ytd !== 0 && <span style={{ color: item.ytd >= 0 ? "#4ade80" : "#f87171" }}>{fmtPct(item.ytd)}</span>}
      </td>
      <td className="px-0.5 py-0.5 w-[40px]">
        {item.sparkline1 && item.sparkline1.length > 2 && <MiniSparkline data={item.sparkline1} color={pctColor} />}
      </td>
    </tr>
  );
}
...
function MiniSparkline({ data, color }: { data: number[]; color: string }) {
  ...
  return <svg width={w} height={h} className="block"><polyline fill="none" stroke={color} strokeWidth="1" points={points} /></svg>;
}

function RegionHeader({ label, count, colors }: { label: string; count: number; colors: typeof bloombergColors.dark }) {
  return (
    <tr><td colSpan={6} className="px-1 py-0.5 text-[9px] font-bold tracking-widest"
      style={{ background: "#0a0a0a", color: colors.accent, borderBottom: `1px solid ${colors.border}` }}>
      {label} <span className="font-mono" style={{ color: colors.textSecondary }}>({count})</span>
    </td></tr>
  );
}
...
function QuoteSummaryBar({ quote, colors }: { quote: Record<string, unknown>; colors: typeof bloombergColors.dark }) {
  ...
  if (chg != null) items.push({ label: "CHG", value: `${isUp ? "+" : ""}${(chg as number).toFixed(2)}`, color: pctColor });
  if (pct != null) items.push({ label: "%CHG", value: fmtPct(pct as number), color: pctColor });
  if (q.regularMarketVolume != null) items.push({ label: "VOL", value: fmtCompact(q.regularMarketVolume as number) });
  if (q.marketCap != null) items.push({ label: "MCAP", value: fmtCompact(q.marketCap as number) });
  if (q.trailingPE != null) items.push({ label: "P/E", value: (q.trailingPE as number).toFixed(1) });
  if (q.beta != null) items.push({ label: "BETA", value: (q.beta as number).toFixed(2) });
  if (q.fiftyTwoWeekLow != null && q.fiftyTwoWeekHigh != null)
    items.push({ label: "52W", value: `${fmtPrice(q.fiftyTwoWeekLow as number)}-${fmtPrice(q.fiftyTwoWeekHigh as number)}` });
  ...
  if (q.epsTrailingTwelveMonths != null) items.push({ label: "EPS", value: `$${(q.epsTrailingTwelveMonths as number).toFixed(2)}` });
  if (q.regularMarketOpen != null) items.push({ label: "OPEN", value: `$${fmtPrice(q.regularMarketOpen as number)}` });
  if (q.regularMarketPreviousClose != null) items.push({ label: "PREV", value: `$${fmtPrice(q.regularMarketPreviousClose as number)}` });

  return (
    <div className="flex flex-wrap gap-x-2 gap-y-0 px-1 py-0.5 text-[9px] font-mono" style={{ background: "#0a0a0a", borderBottom: `1px solid ${colors.border}` }}>
      {items.map((it) => (
        <span key={it.label}>
          <span style={{ color: colors.textSecondary }}>{it.label}:</span>
          <span className="font-bold ml-0.5" style={{ color: it.color ?? colors.text }}>{it.value}</span>
        </span>
      ))}
    </div>
  );
...
export function KeyIndicatorsBar({ data, colors }: {
  data: { americas: MarketItem[]; emea: MarketItem[]; asiaPacific: MarketItem[] }; colors: typeof bloombergColors.dark;
}) {
  const allItems = [...data.americas, ...data.emea, ...data.asiaPacific];
  const indicators = [
    { label: "VIX", id: "VIX" }, { label: "DXY", id: "DOLLAR" }, { label: "US10Y", id: "US 10Y" },
    { label: "GOLD", id: "GOLD" }, { label: "WTI", id: "WTI" }, { label: "BTC", id: "BITCOIN" },
  ];
  return (
      ...
      {indicators.map(({ label, id }) => {
        ...
        return (
          <span key={id} className="whitespace-nowrap">
            ...
            <span className="ml-0.5 font-bold" style={{ color: colors.text }}>{fmtPrice(item.value)}</span>
            <span className="ml-0.5" style={{ color: isUp ? colors.positive : colors.negative }}>{fmtPct(item.pctChange)}</span>
          </span>
        );
      })}
    </div>
...
function ResizeDivider({ onDrag, colors }: {
  onDrag: (deltaX: number) => void; colors: typeof bloombergColors.dark;
}) {
...
function SettingsPanel({ layout, onReorder, onReset, onClose, colors }: {
  ...
  return (
    <div className="fixed z-50 w-52 border shadow-lg p-2 text-xs space-y-2"
      style={{ background: colors.surface, borderColor: colors.border, top: "90px", right: "16px" }}>
      <div className="flex items-center justify-between">
        <span className="text-[9px] font-bold tracking-widest" style={{ color: colors.accent }}>LAYOUT SETTINGS</span>
        <button onClick={onClose}><span className="text-[10px]" style={{ color: colors.textSecondary }}>x</span></button>
      </div>

      <div className="text-[8px] font-bold tracking-wider" style={{ color: colors.textSecondary }}>PANEL ORDER (drag to reorder)</div>
      ...
      <div className="flex gap-1 pt-1 border-t" style={{ borderColor: colors.border }}>
        <button className="flex items-center gap-0.5 text-[9px] px-1.5 py-0.5 font-bold hover:opacity-80"
          style={{ background: "#ef444422", color: "#f87171", border: "1px solid #ef444444" }}
          onClick={onReset}>
          <RotateCcw className="h-2.5 w-2.5" />RESET
        </button>
        <button className="flex items-center gap-0.5 text-[9px] px-1.5 py-0.5 font-bold hover:opacity-80 ml-auto"
          style={{ background: "#22c55e22", color: "#4ade80", border: "1px solid #22c55e44" }}
          onClick={onClose}>
          <Save className="h-2.5 w-2.5" />DONE
        </button>
      </div>
    </div>
  );
...
export function MarketView({ isDarkMode: _ }: MarketViewProps) {
  ...
  const { timePeriod, barInterval, isIntraday: heatmapIsIntraday, handlePeriodChange: handleHeatmapPeriod, handleIntervalChange: handleHeatmapInterval } =
  ...
  const [watchlistHeight, setWatchlistHeight] = useState<number>(() => {
    try { return parseInt(localStorage.getItem(LS_WATCHLIST_H) ?? "220", 10) || 220; } catch { return 220; }
  });
  ...
  const handleWatchlistResizeStart = useCallback((e: React.MouseEvent) => {
    ...
    const onMove = (ev: MouseEvent) => {
      ...
      try { localStorage.setItem(LS_WATCHLIST_H, String(h)); } catch {}
    };
    const onUp = () => {
  ...
  const fearGreedMktQuery    = useQuery<{ history: Array<{ time: string; value: number }> }>({
    queryKey:  ["fear-greed-history", barInterval.startsWith("1") && barInterval.length === 2 ? "3m" : "1y"],
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const chartData = useMemo(() => rawChartData.map((q: any) => ({
    date: q.date,
    price: q.close,
    volume: q.volume ?? 0,
  ...
  // Volume stats
  const maxVolume = useMemo(() => Math.max(...chartData.map((d: { volume: number }) => d.volume), 1), [chartData]);
  ...
  const heatmapOhlcv: OhlcvBar[] = useMemo(() => {
    ...
    const isIntraday = heatmapChartType === "candle"
      ? heatmapIsIntraday
  ...
  const indexToSymbol = useCallback((id: string): string => {
    const map: Record<string, string> = {
      "DOW JONES": "^DJI", "S&P 500": "^GSPC", "NASDAQ": "^IXIC", "RUSSELL 2K": "^RUT",
      "S&P/TSX": "^GSPTSE", "BOVESPA": "^BVSP", "IPC MEXICO": "^MXX",
      "FTSE 100": "^FTSE", "DAX": "^GDAXI", "CAC 40": "^FCHI", "EURO STOXX": "^STOXX50E", "SMI": "^SSMI",
      "NIKKEI": "^N225", "HANG SENG": "^HSI", "SHANGHAI": "000001.SS", "KOSPI": "^KS11",
      "ASX 200": "^AXJO", "SENSEX": "^BSESN", "SET": "^SET.BK",
      "GOLD": "GC=F", "WTI": "CL=F", "BITCOIN": "BTC-USD", "VIX": "^VIX", "DOLLAR": "DX=F", "US 10Y": "^TNX",
    };
    return map[id] ?? id;
  }, []);

  const handleTickSelect = useCallback((item: MarketItem) => {
  ...
  const handleSelectSuggestion = useCallback((symbol: string, name?: string) => {
  ...
  const handleGoToEquity = useCallback(() => {
    if (selectedSymbol) { setStockSymbol(selectedSymbol); setCurrentView("stock"); }
  ...
  useEffect(() => {
    if (!selectedSymbol) {
      if (pins.length > 0) { setSelectedSymbol(pins[0].symbol); setSelectedLabel(pins[0].symbol); }
  ...
  const allMarketItems = [...(marketData?.americas ?? []), ...(marketData?.emea ?? []), ...(marketData?.asiaPacific ?? [])];
      ...
      <div className="flex flex-col overflow-hidden h-full">
        ...
        <div className="shrink-0 flex flex-col" style={{ height: watchlistHeight, minHeight: 100 }}>
          <div className="flex items-center gap-1 px-1 py-0.5 shrink-0"
            ...
            <span className="text-[9px] font-bold tracking-widest" style={{ color: colors.accent }}>WATCHLIST</span>
            <span className="text-[9px] font-mono" style={{ color: colors.textSecondary }}>({pins.length})</span>
            <div className="ml-auto flex items-center gap-1">
              <button className="text-[8px] px-1 hover:opacity-70"
                style={{ color: colors.textSecondary }}
                title="Reset height"
                onClick={() => { setWatchlistHeight(220); try { localStorage.setItem(LS_WATCHLIST_H, "220"); } catch {} }}>
                ↕
              </button>
          ...
          >
            <PinnedAssets onSymbolClick={(sym) => { setSelectedSymbol(sym); setSelectedLabel(sym); }} />
          </div>
        </div>
        ...
        <div
          ...
          style={{ height: 8, background: "#111", borderTop: `1px solid ${colors.border}`, borderBottom: `1px solid ${colors.border}` }}
  ...
  const renderTickDataPanel = (isCollapsed: boolean) => {
    ...
    <div className="flex flex-col overflow-hidden h-full">
      <div className="flex items-center gap-1 px-1 py-0.5 shrink-0"
        ...
        <span className="text-[9px] font-bold tracking-widest" style={{ color: colors.accent }}>TICK DATA</span>
        <span className="text-[9px] font-mono" style={{ color: colors.textSecondary }}>({allMarketItems.length})</span>
        <span className="text-[8px] ml-1">
          <span style={{ color: "#00FF00" }}>▲{upCount}</span>
          <span className="mx-0.5" style={{ color: colors.textSecondary }}>/</span>
          <span style={{ color: "#FF0000" }}>▼{downCount}</span>
        </span>
        <div className="ml-auto flex items-center gap-1">
          <button title="Refresh" className="p-0.5 hover:opacity-70" onClick={refreshData} disabled={marketLoading}>
            {marketLoading
          ...
          <table className="w-full text-[10px] font-mono" style={{ borderCollapse: "collapse" }}>
            <thead>
              <tr style={{ background: "#050505", position: "sticky", top: 0, zIndex: 1 }}>
                <th className="px-1 py-0.5 text-left text-[8px] font-bold tracking-wider" style={{ color: colors.textSecondary }}>INDEX</th>
                <th className="px-1 py-0.5 text-right text-[8px] font-bold tracking-wider" style={{ color: colors.textSecondary }}>LAST</th>
                <th className="px-1 py-0.5 text-right text-[8px] font-bold tracking-wider" style={{ color: colors.textSecondary }}>CHG</th>
                <th className="px-1 py-0.5 text-right text-[8px] font-bold tracking-wider" style={{ color: colors.textSecondary }}>%CHG</th>
                <th className="px-1 py-0.5 text-right text-[8px] font-bold tracking-wider" style={{ color: colors.textSecondary }}>YTD</th>
                <th className="px-1 py-0.5 w-[40px]"></th>
              </tr>
            </thead>
            <tbody>
              <RegionHeader label="AMERICAS" count={marketData?.americas?.length ?? 0} colors={colors} />
              {(marketData?.americas ?? []).map((item: MarketItem) => (
                <TickRow key={item.id} item={item} colors={colors} isSelected={selectedLabel === item.id} onClick={() => handleTickSelect(item)} />
              ))}
              <RegionHeader label="EMEA" count={marketData?.emea?.length ?? 0} colors={colors} />
              {(marketData?.emea ?? []).map((item: MarketItem) => (
                <TickRow key={item.id} item={item} colors={colors} isSelected={selectedLabel === item.id} onClick={() => handleTickSelect(item)} />
              ))}
              <RegionHeader label="ASIA PACIFIC" count={marketData?.asiaPacific?.length ?? 0} colors={colors} />
  ...
  const renderChartPanel = (_isCollapsed: boolean) => (
    <div className="flex flex-col h-full">
      ...
      <div className="relative shrink-0">
        <div className="flex items-center gap-1 px-1 py-0.5"
          ...
          <input ref={searchRef}
            ...
            style={{ background: "#000", color: colors.accent, borderColor: showDropdown ? colors.accent : colors.border }}
            placeholder="SYMBOL <GO>" value={searchInput}
            onChange={(e) => { setSearchInput(e.target.value.toUpperCase()); setShowDropdown(true); }}
            onFocus={() => setShowDropdown(true)}
            onBlur={() => setTimeout(() => { setShowDropdown(false); setDropdownIdx(-1); }, 150)}
            onKeyDown={(e) => {
              const apiItems: any[] = (searchResult.data as any[]) ?? [];
              const items = searchInput.length > 0
                ? apiItems
          ...
          <button className="text-[9px] px-1.5 py-0.5 font-bold"
            style={{ background: colors.accent, color: "#000" }} onClick={handleSearchSubmit}>GO</button>
        </div>
        ...
        {showDropdown && (() => {
          ...
          const showEmpty = isTyping && !searchResult.isLoading && apiItems.length === 0 && searchQuery.length > 0;
          if (!showRecent && !showResults && !showLoading && !showEmpty) return null;
          return (
            <div className="absolute top-full left-0 right-0 z-50 border border-t-0 shadow-xl"
              ...
              {showRecent && (<>
                <div className="flex items-center justify-between px-2 py-0.5"
                  ...
                  <span className="text-[8px] font-bold tracking-widest" style={{ color: colors.textSecondary }}>RECENT</span>
                  <button className="text-[8px] hover:opacity-70" style={{ color: colors.textSecondary }}
                    onMouseDown={(e) => {
                      e.preventDefault();
                      setRecentSymbols([]);
                ...
                {recentSymbols.map((r, idx) => (
                  <button key={r.symbol} type="button"
                    ...
                    style={{ borderColor: colors.border, color: colors.text, background: dropdownIdx === idx ? "#0a1628" : "transparent" }}
                    onMouseEnter={() => setDropdownIdx(idx)}
                    onMouseDown={() => handleSelectSuggestion(r.symbol, r.name)}>
                    <span className="font-bold font-mono w-16 shrink-0" style={{ color: colors.accent }}>{r.symbol}</span>
                    <span className="truncate flex-1 text-[9px]" style={{ color: colors.textSecondary }}>{r.name !== r.symbol ? r.name : ""}</span>
                    <span className="text-[8px] ml-auto" style={{ color: colors.textSecondary + "66" }}>↩</span>
                  </button>
                ))}
              </>)}
              ...
              {/* Loading */}
              {showLoading && (
                <div className="px-2 py-2 flex items-center gap-1.5 text-[10px]" style={{ color: colors.textSecondary }}>
              ...
              {showResults && (<>
                <div className="px-2 py-0.5"
                  style={{ borderBottom: `1px solid ${colors.border}`, background: "#0a0a0a" }}>
                  <span className="text-[8px] font-bold tracking-widest" style={{ color: colors.textSecondary }}>
                ...
                <div className="max-h-48 overflow-y-auto">
                  {apiItems.map((item: any, idx: number) => (
                    <button key={item.symbol} type="button"
                      ...
                      style={{ borderColor: colors.border, color: colors.text, background: dropdownIdx === idx ? "#0a1628" : "transparent" }}
                      onMouseEnter={() => setDropdownIdx(idx)}
                      onMouseDown={() => handleSelectSuggestion(item.symbol, item.shortname ?? item.longname)}>
                      <span className="font-bold font-mono w-16 shrink-0" style={{ color: colors.accent }}>{item.symbol}</span>
                      <span className="truncate flex-1">{item.shortname ?? item.longname}</span>
                      <span className="ml-auto shrink-0 text-[9px]" style={{ color: colors.textSecondary }}>{item.exchDisp}</span>
                    </button>
                  ))}
                </div>
              </>)}
      ...
      {/* Symbol header */}
      <div className="px-1 py-0.5 shrink-0 flex items-center gap-2"
            ...
            {quote && (
              <>
                ...
                <span className="text-xs font-bold font-mono"
                  style={{ color: (quote.regularMarketChangePercent ?? 0) >= 0 ? "#00FF00" : "#FF0000" }}>
                  {(quote.regularMarketChangePercent ?? 0) >= 0 ? "▲" : "▼"}
                  {fmtPct(quote.regularMarketChangePercent ?? 0)}
                </span>
                {quote.shortName && (
                  <span className="text-[9px] truncate" style={{ color: colors.textSecondary }}>{quote.shortName}</span>
                )}
                <MarketSessionBadge state={quote.marketState as string | undefined} />
              </>
            )}
            <button className="ml-auto text-[8px] px-1 py-0 border hover:opacity-70"
              style={{ borderColor: "#00FFFF44", color: "#00FFFF" }} onClick={handleGoToEquity}>FULL EQTY →</button>
          </>
        ) : (
          <span className="text-[10px]" style={{ color: colors.textSecondary }}>Select an index or search a symbol</span>
        )}
      </div>

      {/* Quote details */}
      {quote && <QuoteSummaryBar quote={quote} colors={colors} />}
      {quote && (
        <div className="px-2 py-0.5 shrink-0" style={{ background: "#050505", borderBottom: `1px solid ${colors.border}` }}>
          <ExtendedHoursPrice quote={quote} positiveColor="#00FF00" negativeColor="#FF0000" />
        </div>
      )}

      {/* Time period tabs + chart type + volume toggle */}
      <div className="flex items-center gap-0 px-1 py-0.5 shrink-0"
        style={{ background: "#050505", borderBottom: `1px solid ${colors.border}` }}>
        {TIME_PERIODS.map((p) => {
          const active = p === timePeriod;
          const validRanges = heatmapChartType === "candle" ? INTERVAL_VALID_RANGES[barInterval as BarInterval] : null;
          const disabled = validRanges ? !validRanges.includes(p as TimePeriod) : false;
          return (
            <button key={p} className="text-[9px] font-bold font-mono px-1.5 py-0.5"
              disabled={disabled}
              style={{
                background: active ? colors.accent : "transparent",
                color: disabled ? `${colors.textSecondary}44` : active ? "#000" : colors.textSecondary,
        ...
        <div className="ml-auto flex items-center gap-1">
          ...
          {heatmapChartType === "area" && (
            <button className="text-[8px] px-1 py-0 font-bold"
              style={{ color: showVolume ? "#00FFFF" : colors.textSecondary, background: showVolume ? "#00FFFF15" : "transparent" }}
          ...
          {heatmapChartType === "area" && (
            <button className="text-[8px] px-1 py-0 font-bold"
              style={{ color: showMACD ? "#ff9800" : colors.textSecondary, background: showMACD ? "#ff980015" : "transparent" }}
              onClick={() => setShowMACD((v) => !v)}>
              MACD
            </button>
          )}
          {quoteQuery.isLoading && <Loader2 className="h-2.5 w-2.5 animate-spin" style={{ color: colors.accent }} />}
        </div>
      </div>
      ...
      {heatmapChartType === "candle" && (
        <div className="flex items-center gap-1 px-1 py-0.5 shrink-0"
          ...
          {heatmapOhlcv.some(d => (d.volume ?? 0) > 0) && (
            <button className="text-[8px] px-1 py-0 font-bold border"
              style={{
      ...
      {heatmapChartType === "candle" && (
        <div className="flex items-center gap-1 px-1 py-0.5 shrink-0"
          style={{ background: "#050505", borderBottom: `1px solid ${colors.border}` }}>
          <span className="text-[9px] font-mono mr-1" style={{ color: colors.textSecondary }}>TF</span>
      ...
      {/* Chart area */}
      <div className="flex-1 min-h-0 overflow-hidden px-0.5 py-0.5" style={{ background: "#050505" }}>
        {!selectedSymbol ? (
          <div className="flex items-center justify-center h-full">
            <div className="text-center">
              <BarChart2 className="h-8 w-8 mx-auto mb-2 opacity-10" style={{ color: colors.textSecondary }} />
              <div className="text-[10px]" style={{ color: colors.textSecondary }}>Click a market index or search a symbol</div>
            </div>
          </div>
          ...
          <div className="flex items-center justify-center h-full">
            <span className="text-[10px]" style={{ color: colors.textSecondary }}>No data for this period</span>
          </div>
        ) : heatmapChartType === "candle" ? (
          ...
          <div className="h-full flex flex-col">
            ...
            {/* Chart footer stats */}
            <div className="shrink-0 flex justify-between text-[9px] font-mono px-1 py-0.5"
              style={{ borderTop: "1px solid #1a1a1a", color: colors.textSecondary }}>
              <span>O:<span style={{ color: colors.text }}>${fmtPrice(chartData[0]?.price)}</span></span>
              <span>H:<span style={{ color: colors.text }}>${fmtPrice(Math.max(...chartData.map((d: { price: number }) => d.price)))}</span></span>
              <span>L:<span style={{ color: colors.text }}>${fmtPrice(Math.min(...chartData.map((d: { price: number }) => d.price)))}</span></span>
              <span>C:<span style={{ color: colors.text }}>${fmtPrice(chartData[chartData.length - 1]?.price)}</span></span>
          ...
          <div className="h-full flex flex-col">
            ...
            <div style={{ flex: showMACD ? "7" : "1", minHeight: 0 }}>
              <ResponsiveContainer width="100%" height="100%">
                <ComposedChart data={chartData} margin={{ top: 5, right: 5, left: 0, bottom: 0 }}>
                  ...
                  <Tooltip
                    contentStyle={{
                      backgroundColor: "#111", borderColor: colors.border, color: colors.text,
                      fontSize: 10, fontFamily: "monospace", borderRadius: 0, padding: "4px 6px",
                    }}
                  ...
                  {showVolume && (
                    ...
                    >
                      {chartData.map((entry: { volume: number }, idx: number) => (
                        <Cell
                          key={idx}
                          fill={entry.volume > avgVolume * 1.5 ? "#ff990044" : entry.volume > avgVolume ? "#4ade8033" : "#33333344"}
                        />
                      ))}
                    </Bar>
                  )}
            ...
            {showMACD && (
              <div style={{ flex: "3", minHeight: 0, borderTop: "1px solid #1a1a1a" }}>
                {/* MACD label */}
                <div className="flex items-center gap-2 px-1" style={{ height: 14, background: "#050505" }}>
                  <span className="text-[8px] font-bold font-mono" style={{ color: "#ff9800" }}>MACD</span>
                  <span className="text-[7px] font-mono" style={{ color: colors.textSecondary }}>12,26,9</span>
                  {macdData.length > 0 && macdData[macdData.length - 1].macd != null && (
                    <>
                      ...
                      <span className="text-[7px] font-mono"
                        style={{ color: (macdData[macdData.length - 1].histogram ?? 0) >= 0 ? "#4caf50" : "#ef5350" }}>
                ...
                <div style={{ height: "calc(100% - 14px)" }}>
                  <ResponsiveContainer width="100%" height="100%">
                    <ComposedChart data={macdData} margin={{ top: 2, right: 5, left: 0, bottom: 0 }}>
                      <CartesianGrid strokeDasharray="3 3" stroke="#1a1a1a" />
                      <XAxis
                        dataKey="label"
                        tick={{ fontSize: 8, fill: colors.textSecondary }}
                      ...
                      <Tooltip
                        ...
                        formatter={(v: unknown, name: string) => {
                          ...
                          const labels: Record<string, string> = { macd: "MACD", signal: "Signal", histogram: "Hist" };
                      ...
                      <Bar dataKey="histogram" isAnimationActive={false}
                        ...
                        {macdData.map((entry, idx) => (
                          <Cell
                            key={idx}
                            fill={(entry.histogram ?? 0) >= 0
                              ? ((idx > 0 && (entry.histogram ?? 0) >= (macdData[idx - 1].histogram ?? 0)) ? "#4caf50" : "#4caf5080")
                              : ((idx > 0 && (entry.histogram ?? 0) <= (macdData[idx - 1].histogram ?? 0)) ? "#ef5350" : "#ef535080")
                            }
                          />
                        ))}
                      </Bar>
            ...
            <div className="shrink-0 flex justify-between text-[9px] font-mono px-1 py-0.5"
              ...
              {showVolume && avgVolume > 0 && (
                <span>AvgVol:<span style={{ color: colors.text }}>{fmtVolShort(avgVolume)}</span></span>
              )}
              <span style={{ color: chartColor }}>
                {chartTrend ? "▲" : "▼"}
                {Math.abs(((chartData[chartData.length - 1].price - chartData[0].price) / chartData[0].price) * 100).toFixed(2)}%
  ...
  return (
    <div className="h-full flex flex-col overflow-hidden relative" style={{ backgroundColor: "#000", color: colors.text }}>
      {showSettings && (
        <SettingsPanel
          layout={layout}
          onReorder={(order) => updateLayout({ panelOrder: order })}
          onReset={() => { localStorage.removeItem(LS_LAYOUT_KEY); setLayout(DEFAULT_LAYOUT); saveLayout(DEFAULT_LAYOUT); }}
      ...
      <div ref={containerRef} className="flex-1 flex overflow-hidden min-h-0">
        {layout.panelOrder.map((panelId, idx) => {
          ...
          return (
            <div key={panelId} className="flex" style={{ width, minWidth: isCollapsed ? 36 : (panelId === "watchlist" ? 260 : undefined), flexShrink: isCollapsed ? 0 : 1, flexGrow: isCollapsed ? 0 : 1 }}>
              {/* Panel content */}
              <div className="flex-1 overflow-hidden">
                {panelRenderers[panelId](isCollapsed)}
              </div>
