...
const CONSUMER_KEYS = ["breakeven_5y", "breakeven_10y", "mortgage_30y", "cc_delinquency", "mtg_delinquency"] as const;

// Signals that have a threshold (shown in overview alert grid)
const SIGNAL_KEYS = ["hy_spread", "ig_spread", "em_hy_spread", "ted_spread", "vix", "stl_fsi", "nfci", "yield_10y2y", "yield_10y3m"] as const;
type SignalKey = typeof SIGNAL_KEYS[number];

// ── Level config ──────────────────────────────────────────────────────────────

const LEVEL_CFG = {
  0: { label: "NORMAL",       color: "#4caf50", bg_dark: "#051a05", bg_light: "#f0fff0", Icon: ShieldCheck },
  1: { label: "WATCH",        color: "#ffc107", bg_dark: "#1a1500", bg_light: "#fffde0", Icon: ShieldAlert },
  2: { label: "WARNING",      color: "#ff9800", bg_dark: "#1a0c00", bg_light: "#fff3e0", Icon: AlertTriangle },
  3: { label: "CRISIS ALERT", color: "#ef5350", bg_dark: "#1a0505", bg_light: "#fff0f0", Icon: XCircle },
} as const;
...
function fmtDate(d: string) {
  ...
  const mon = months[(parseInt(parts[1]) - 1) % 12];
  // Show day only when it's meaningful (not the 1st, which is the monthly default)
  if (parts[2] && parts[2] !== "01") return `${mon} ${parseInt(parts[2])}`;
...
function SectionHeader({ title, sub, colors }: { title: string; sub?: string; colors: any }) {
  return (
...
function CrisisBanner({ data, isDark, colors }: { data: CreditData; isDark: boolean; colors: any }) {
      ...
      <div>
        ...
        <div className="text-[10px] font-mono" style={{ color: colors.text }}>
          ...
          {data.triggered.map(k => {
            const s = (data.signals as any)[k] as CreditSignal;
            return s ? s.label : k;
          }).join(" · ")}
        </div>
      </div>
...
function LevelGauge({ level, triggered, total, isDark, colors }: {
  level: CrisisLevel; triggered: number; total: number; isDark: boolean; colors: any;
}) {
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2">
        ...
        <span className="text-[10px] tracking-widest font-bold" style={{ color: colors.textSecondary }}>
          CRISIS LEVEL
        </span>
      </div>

      <div className="flex items-baseline gap-2">
        <span className="text-3xl font-bold font-mono" style={{ color: cfg.color }}>{level}</span>
        <span className="text-sm font-bold font-mono" style={{ color: cfg.color }}>{cfg.label}</span>
      </div>
      ...
      <div className="flex justify-between text-[8px] font-mono" style={{ color: colors.textSecondary }}>
        <span>NORMAL</span><span>WATCH</span><span>WARNING</span><span>CRISIS</span>
      </div>
    </div>
  );
}
...
function SignalCard({ sig, isDark, colors }: {
  sig: CreditSignal; isDark: boolean; colors: any;
}) {
  if (!sig) return (
    <div className="border p-2 opacity-40 font-mono text-center text-xs"
      style={{ borderColor: colors.border, backgroundColor: colors.surface, color: colors.textSecondary }}>
      No data
    </div>
  );

  const pct = sig.threshold != null
    ? Math.min(100, Math.abs(sig.value / sig.threshold) * 100)
    : null;
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between">
        <span className="text-[9px] tracking-wide font-bold" style={{ color: colors.textSecondary }}>
      ...
      <div className="flex items-baseline gap-1">
        <span className="text-[11px] font-bold" style={{ color: sig.triggered ? "#ef5350" : colors.text }}>
...
function SeriesChart({ sig, isDark, colors, height = 200 }: {
  sig: CreditSignal; isDark: boolean; colors: any; height?: number;
}) {
  if (!sig || sig.series.length < 2) {
    return (
      <div className="flex items-center justify-center border" style={{ height, borderColor: colors.border, color: colors.textSecondary }}>
  ...
  const ttStyle   = { backgroundColor: isDark ? "#1a1a1a" : "#fff", border: `1px solid ${colors.border}`, fontSize: 11, fontFamily: "monospace" };

  return (
    <div className="border p-2" style={{ borderColor: colors.border, backgroundColor: colors.surface }}>
      <div className="flex items-center justify-between mb-1">
        <span className="text-[10px] font-bold tracking-wide font-mono" style={{ color: colors.accent }}>
          {sig.label.toUpperCase()}
        </span>
        <div className="flex items-center gap-2">
          <span
      ...
      <ResponsiveContainer width="100%" height={height}>
        <AreaChart data={chartData} margin={{ top: 5, right: 5, left: 0, bottom: 15 }}>
          ...
          <XAxis
            ...
            angle={-45} textAnchor="end" interval={Math.floor(chartData.length / 6)}
          ...
          <Tooltip
            ...
            contentStyle={ttStyle} labelStyle={{ color: colors.text }}
          />
          {sig.threshold != null && (
            <ReferenceLine
              y={sig.threshold}
              stroke="#888" strokeDasharray="4 2"
              label={{ value: `Threshold ${sig.threshold}${sig.unit === "bps" ? " bps" : sig.unit}`, position: "insideTopRight", fontSize: 8, fill: "#888" }}
            />
          )}
...
function OverviewTab({ data, isDark, colors }: { data: CreditData; isDark: boolean; colors: any }) {
    ...
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
        <div className="sm:col-span-1">
          <LevelGauge level={data.level} triggered={triggered} total={totalSignals} isDark={isDark} colors={colors} />
        </div>

        <div className="sm:col-span-2 grid grid-cols-2 gap-2">
          {/* Triggered signals list */}
          <div className="border p-2" style={{ borderColor: colors.border, backgroundColor: colors.surface }}>
            <div className="text-[9px] tracking-widest font-bold mb-1" style={{ color: colors.textSecondary }}>
              TRIGGERED SIGNALS
            </div>
            {triggered === 0 ? (
                ...
                {data.triggered.map(k => {
                  ...
                  return (
                    <div key={k} className="flex items-center gap-1.5 text-[10px] font-mono">
                      ...
                      <span className="ml-auto font-bold" style={{ color: "#f87171" }}>{fmtVal(s.value, s.unit)}</span>
                    </div>
                  );
                })}
              </div>
          ...
          {/* Key spread summary */}
          <div className="border p-2 space-y-1.5" style={{ borderColor: colors.border, backgroundColor: colors.surface }}>
            <div className="text-[9px] tracking-widest font-bold mb-2" style={{ color: colors.textSecondary }}>
              KEY SPREADS
            </div>
            {SPREAD_KEYS.map(k => {
              const s = (data.signals as any)[k] as CreditSignal;
              ...
              return (
                <div key={k} className="flex items-center justify-between text-[10px] font-mono">
                  ...
                  <span className="font-bold" style={{ color: s.triggered ? "#ef5350" : colors.text }}>
      ...
      <div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-2">
          {SIGNAL_KEYS.map(k => (
            <SignalCard
              key={k}
              sig={(data.signals as any)[k] as CreditSignal}
              isDark={isDark}
              colors={colors}
            />
          ))}
      ...
      <div
        ...
        style={{ borderColor: colors.border, backgroundColor: colors.surface, color: colors.textSecondary }}
      >
        <span>Source: <span style={{ color: colors.accent }}>FRED</span> (Federal Reserve Economic Data) · Level = signals above threshold</span>
      </div>
    </div>
  );
}
...
function SpreadsTab({ data, isDark, colors }: { data: CreditData; isDark: boolean; colors: any }) {
  return (
    <div className="space-y-2">
      <SectionHeader title="BOND SPREADS" sub="Option-Adjusted Spread (bps) — higher = more credit stress" colors={colors} />
          ...
          <table className="w-full text-[11px] font-mono">
            ...
            <tbody>
              {SPREAD_KEYS.map((k, i) => {
                const s = (data.signals as any)[k] as CreditSignal;
                if (!s) return null;
                const pctOfThreshold = s.threshold ? Math.round((s.value / s.threshold) * 100) : null;
                return (
                  <tr
                    key={k}
                    style={{
                  ...
                  >
                    <td className="px-3 py-2" style={{ color: colors.text }}>{s.label}</td>
                    <td className="px-3 py-2 text-right font-bold" style={{ color: s.triggered ? "#ef5350" : colors.text }}>
                      {fmtVal(s.value, s.unit)}
                    </td>
                    ...
                    <td className="px-3 py-2 text-right" style={{ color: pctOfThreshold != null && pctOfThreshold >= 80 ? "#ef5350" : colors.textSecondary }}>
                      {pctOfThreshold != null ? `${pctOfThreshold}%` : "—"}
                    </td>
                    <td className="px-3 py-2 text-center">
                      {s.triggered
                        ? <span className="font-bold" style={{ color: "#f87171" }}>⚠ TRIGGERED</span>
...
function StressTab({ data, isDark, colors }: { data: CreditData; isDark: boolean; colors: any }) {
  return (
    <div className="space-y-2">
      <SectionHeader title="FINANCIAL STRESS INDICATORS" sub="VIX · FSI · NFCI · Yield curve inversions" colors={colors} />
          ...
          return (
            <div
              ...
              style={{
                ...
                backgroundColor: inverted ? (isDark ? "#1a0505" : "#fff5f5") : (isDark ? "#051a05" : "#f5fff5"),
              }}
            >
              {inverted
              ...
              <div>
                <div className="text-xs font-bold" style={{ color: inverted ? "#ef5350" : "#4caf50" }}>
      ...
      <div
        ...
        style={{ borderColor: colors.accent + "44", backgroundColor: colors.surface, color: colors.textSecondary }}
      >
...
function ConsumerTab({ data, isDark, colors }: { data: CreditData; isDark: boolean; colors: any }) {
  return (
    <div className="space-y-2">
      <SectionHeader title="CONSUMER & HOUSING STRESS" sub="Breakeven inflation · Mortgage rate · Delinquency rates" colors={colors} />
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-2">
        {CONSUMER_KEYS.map(k => (
          <SeriesChart
            key={k}
...
export function CreditView({ onBack }: { onBack: () => void }) {
  ...
  return (
    <div className="h-full overflow-y-auto" style={{ backgroundColor: colors.background, color: colors.text }}>
      ...
      <div className="flex items-center gap-1 px-2 py-1.5 border-b" style={{ borderColor: colors.border }}>
      ...
      <div className="p-2">
        {isLoading && !data && (
          <div className="flex items-center justify-center py-20" style={{ color: colors.textSecondary }}>
            <RefreshCw className="h-5 w-5 animate-spin mr-3" style={{ color: colors.accent }} />
            <span className="text-[10px] font-mono">Fetching stress indicators from FRED… (first load ~10s)</span>
          </div>
        )}

        {error && !data && (
          <div className="flex items-center gap-2 p-2 border text-[10px] font-mono" style={{ borderColor: "#ef5350", color: "#ef5350" }}>
        ...
        {data && (
          <>
            {activeTab === "overview"  && <OverviewTab  data={data} isDark={isDark} colors={colors} />}
            ...
            {activeTab === "consumer"  && <ConsumerTab  data={data} isDark={isDark} colors={colors} />}
          </>
        )}
      </div>
    </div>
