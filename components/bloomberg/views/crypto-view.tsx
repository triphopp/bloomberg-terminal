...
async function fetchCryptoHistory(
  ...
  const res = await fetch(
    `/api/crypto?type=history&coin=${encodeURIComponent(coin)}&period=${yfPeriod}&interval=${interval}`
  );
  if (!res.ok) throw new Error(`HTTP ${res.status}`);
            ...
            <>
              ...
              >
                ...
                <div className="flex items-baseline gap-4 mt-1">
                  ...
                  <span className="text-[11px] font-mono font-bold" style={{ color: priceColor }}>
                    {isUp ? "+" : ""}
                    {(selected.pctChange ?? 0).toFixed(2)}%
                  </span>
                  <span className="text-[9px] font-mono" style={{ color: colors.textSecondary }}>
                        ...
                        >
                          ...
                          <Bar dataKey="volume" isAnimationActive={false}>
                            {chartData.map((entry, i) => (
                              <Cell
                                // Recharts <Cell> has no stable identity of its own;
                                // the bar's timestamp is unique per row.
                                key={entry.date}
                                fill={
