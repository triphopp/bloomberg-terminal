...
import { fmt, fmtK, type Colors } from "../helpers";
...
export function SellModal({ target, colors, onClose, onSold }: Props) {
  const [sellPrice,   setSellPrice]   = useState(
    target.current_price != null
      ? parseFloat(target.current_price.toFixed(4)).toString()
      : ""
  );
  ...
  const handleSell = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const vol = sellPartial ? parseFloat(sellVolume) : 0;
  ...
  const sellDisabled = loading || !sellDate || !sellPrice || parseFloat(sellPrice) <= 0
    || (sellPartial && (!sellVolume || parseFloat(sellVolume) <= 0));

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: "rgba(0,0,0,0.7)" }}
      onClick={onClose}>
      <div className="border p-4 w-[360px] max-h-[90vh] overflow-y-auto"
        ...
        {/* Position summary */}
        <div className="text-[9px] font-mono mb-3 p-2 border" style={{ borderColor: colors.border, background: "#050505" }}>
          <div className="flex justify-between">
            <span style={{ color: colors.textSecondary }}>Position</span>
            <span style={{ color: colors.text }}>{target.symbol} × {target.volume.toLocaleString()}</span>
          </div>
          <div className="flex justify-between">
            <span style={{ color: colors.textSecondary }}>Avg Entry</span>
            <span style={{ color: colors.text }}>{target.acc_currency === "USD" ? "$" : "฿"}{fmt(target.price_entry, 4)}</span>
          </div>
          <div className="flex justify-between">
            <span style={{ color: colors.textSecondary }}>Current</span>
            <span style={{ color: target.current_price != null && target.current_price >= target.price_entry ? "#4ade80" : "#f87171" }}>
              {target.current_price != null ? `${target.acc_currency === "USD" ? "$" : "฿"}${fmt(target.current_price, 4)}` : "—"}
            </span>
          </div>
        </div>

        {/* Partial toggle */}
        <label className="flex items-center gap-2 mb-3 cursor-pointer text-[9px] font-mono"
        ...
        {sellPartial && (
          <div className="mb-2">
            <label className="text-[8px] font-mono" style={{ color: colors.textSecondary }}>
        ...
        {/* Sell price */}
        <div className="mb-2">
          <label className="text-[8px] font-mono" style={{ color: colors.textSecondary }}>Sell Price</label>
          <input
            type="number" step="any"
            className="w-full px-2 py-1 text-[10px] font-mono border mt-0.5 outline-none"
            style={{ background: "#050505", borderColor: colors.border, color: colors.text }}
            ...
            onChange={e => {
              ...
              const dot = v.indexOf(".");
              setSellPrice(dot >= 0 && v.length - dot - 1 > 4 ? parseFloat(v).toFixed(4) : v);
            }}
        ...
        {/* Sell date */}
        <div className="mb-3">
          <label className="text-[8px] font-mono" style={{ color: colors.textSecondary }}>Sell Date</label>
          <input
            type="date"
            className="w-full px-2 py-1 text-[10px] font-mono border mt-0.5 outline-none"
            style={{ background: "#050505", borderColor: colors.border, color: colors.text }}
        ...
        {sellPrice && (
          <div className="text-[9px] font-mono mb-3 p-2 border" style={{ borderColor: colors.border, background: "#050505" }}>
            <div className="flex justify-between">
              ...
              {(() => {
                const ep = parseFloat(sellPrice) || 0;
                const vol = sellPartial ? (parseFloat(sellVolume) || 0) : target.volume;
                const pnl = (ep - target.price_entry) * vol;
                return (
                  <span className="font-bold" style={{ color: pnl >= 0 ? "#4ade80" : "#f87171" }}>
                    {pnl >= 0 ? "+" : ""}{fmt(pnl, 2)} {target.acc_currency === "USD" ? "USD" : "THB"}
                  </span>
                );
              })()}
            </div>
