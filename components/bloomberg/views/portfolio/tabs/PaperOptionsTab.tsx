...
interface OptionPosition {
  ...
  option_type: string; quantity: number; entry_price: number;
  entry_date: string; status: string; commission: number;
  exit_price?: number; realized_pnl?: number; notes: string;
  spot_price?: number; live_price?: number; market_value?: number;
  unrealized_pnl?: number; unrealized_pnl_pct?: number;
  dte?: number; direction: string;
}
...
export function PaperOptionsTab({ colors }: { colors: Colors }) {
  ...
  const loadPositions = useCallback(async () => {
    ...
    try {
      const [openR, closedR] = await Promise.all([
        fetch(`/api/paper/accounts/${accountId}/options`),
        fetch(`/api/paper/accounts/${accountId}/options/closed`),
      ]);
      setPositions(await openR.json());
      setClosedPositions(await closedR.json());
    } catch { /* */ } finally { setLoading(false); }
  }, [accountId]);

  useEffect(() => { loadPositions(); }, [accountId, loadPositions]);
  ...
  const submitOrder = async () => {
    ...
    try {
      const body: Record<string, unknown> = {
        ...
        expiry, strike: parseFloat(strike),
      ...
      if (!r.ok) { setError(d.detail || "Order failed"); return; }

      const action = d.action || "done";
      const priceStr = d.fill_price ? ` @ $${fmt(d.fill_price, 2)}` : "";
      setResult(`${action.toUpperCase()}: ${underlying.toUpperCase()} ${optionType} ${strike}${priceStr}`);
      setUnderlying(""); setExpiry(""); setStrike(""); setLimitPrice(""); setNotes("");
      loadPositions();
  ...
  const expirePosition = async (posId: string) => {
    try {
      ...
      if (!r.ok) { alert("Expire failed"); return; }
      loadPositions();
    } catch { /* */ }
  };
  ...
  const totalValue = positions.reduce((s, p) => s + (p.market_value ? Math.abs(p.market_value) : 0), 0);
      ...
      <div className="flex items-center gap-1 px-2 py-1 border-b shrink-0" style={{ borderColor: colors.border }}>
        ...
        {(["positions", "trade", "closed"] as const).map(t => (
          <button key={t}
            ...
            {t === "positions" ? `OPEN (${positions.length})` :
             t === "trade" ? "NEW ORDER" : `CLOSED (${closedPositions.length})`}
          </button>
        ))}
        <span className="text-[8px] ml-1" style={{ color: pnlColor(totalUnrealized) }}>
          Unreal: {fmtK(totalUnrealized)}
        </span>
        <button className="ml-auto p-0.5" onClick={loadPositions}>
          ...
          <div className="p-2 space-y-1.5">
            <div className="border p-2" style={{ borderColor: colors.border }}>
              ...
              <div className="flex items-center gap-1 flex-wrap">
                <input placeholder="UNDERLYING" value={underlying}
                  onChange={e => setUnderlying(e.target.value)}
                  className={`${inputCls} w-16 uppercase`} style={inputStyle} />

                <input placeholder="EXPIRY" type="date" value={expiry}
                  onChange={e => setExpiry(e.target.value)}
                  className={`${inputCls} w-28`} style={inputStyle} />

                <input placeholder="STRIKE" value={strike}
                  onChange={e => setStrike(e.target.value)}
                  className={`${inputCls} w-16`} style={inputStyle} type="number" />
                ...
                <div className="flex border" style={{ borderColor: colors.border }}>
                  {(["call", "put"] as const).map(t => (
                    <button key={t}
                      className="px-2 py-0.5 text-[9px] font-bold"
                      style={{
                        background: optionType === t
                          ? (t === "call" ? "#0066CC" : "#CC6600")
                          : "transparent",
                ...
                <input placeholder="QTY" value={quantity}
                  ...
                  className={`${inputCls} w-12`} style={inputStyle} type="number" min="1" />

                <input placeholder="Limit $ (opt)" value={limitPrice}
                  onChange={e => setLimitPrice(e.target.value)}
                  className={`${inputCls} w-20`} style={inputStyle} type="number" />
              </div>
              ...
              {result && <div className="text-[9px] mt-1" style={{ color: "#00FF00" }}>{result}</div>}
              {error && <div className="text-[9px] mt-1" style={{ color: "#FF4444" }}>{error}</div>}

              <div className="text-[7px] opacity-30 mt-1">
                BUY = long (pay premium) · SELL = short/write (receive premium, unlimited risk on calls)
                · 1 contract = 100 shares · Commission: $0.65/contract
              </div>
            </div>
          </div>
        )}
              ...
              <tbody>
                {positions.map(p => (
                  <tr key={p.id} className="border-b hover:bg-white/5"
                    ...
                    <td className="px-1 text-right font-mono" style={{
                      color: p.dte != null && p.dte <= 7 ? "#FF4444" :
                             p.dte != null && p.dte <= 21 ? "#ff9900" : colors.text
                    }}>
                      {p.dte ?? "—"}
                    </td>
                    <td className="px-1 font-bold" style={{
                    ...
                    <td className="px-1 flex items-center gap-0.5">
                      ...
                      {p.dte != null && p.dte <= 0 && (
                        <button onClick={() => expirePosition(p.id)}
                          className="px-1 py-0.5 text-[8px] border hover:opacity-80"
              ...
              <tbody>
                {closedPositions.map(p => (
                  <tr key={p.id} className="border-b" style={{ borderColor: `${colors.border}44` }}>
                    <td className="px-2 py-0.5 font-bold" style={{ color: colors.accent }}>{p.underlying}</td>
                    <td className="px-1" style={{
                      color: p.option_type === "call" ? "#4488FF" : "#FF8844"
                    }}>
                      {p.option_type.toUpperCase()}
                    ...
                    <td className="px-1 text-right font-mono" style={{ color: pnlColor(p.realized_pnl) }}>
                      {p.realized_pnl != null ? fmtK(p.realized_pnl) : "—"}
                    </td>
                    <td className="px-1 font-bold text-[8px]" style={{
                      color: p.status === "closed" ? "#ff9900" :
                             p.status === "exercised" ? "#00FF00" : "#FF4444"
                    }}>
