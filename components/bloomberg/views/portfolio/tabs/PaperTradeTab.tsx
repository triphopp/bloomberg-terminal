...
export function PaperTradeTab({ colors }: { colors: Colors }) {
  ...
  const loadPending = useCallback(async () => {
    if (!accountId) return;
    try {
      const r = await fetch(`/api/paper/accounts/${accountId}/orders?status=pending`);
      setPendingOrders(await r.json());
    } catch { /* */ }
  }, [accountId]);

  useEffect(() => { loadPending(); }, [accountId, loadPending]);

  const submit = async () => {
    setError(null); setResult(null);
    if (!accountId || !symbol || !quantity) { setError("Fill required fields"); return; }
    setSubmitting(true);
    try {
      ...
      <div className="border p-2 space-y-1.5" style={{ borderColor: colors.border }}>
        ...
        <div className="flex items-center gap-1 flex-wrap">
          ...
          <input placeholder="SYMBOL" value={symbol} onChange={e => setSymbol(e.target.value)}
            className={`${inputCls} w-20 uppercase`} style={inputStyle}
          ...
          {(orderType === "limit" || orderType === "stop_limit") && (
            <input placeholder="Limit $" value={limitPrice} onChange={e => setLimitPrice(e.target.value)}
              className={`${inputCls} w-20`} style={inputStyle} type="number" />
          )}
          {(orderType === "stop" || orderType === "stop_limit") && (
            <input placeholder="Stop $" value={stopPrice} onChange={e => setStopPrice(e.target.value)}
              className={`${inputCls} w-20`} style={inputStyle} type="number" />
          )}
        ...
        {result && <div className="text-[9px]" style={{ color: "#00FF00" }}>{result}</div>}
        {error && <div className="text-[9px]" style={{ color: "#FF4444" }}>{error}</div>}
      </div>

      {/* Pending orders */}
      <div className="border flex-1 overflow-y-auto" style={{ borderColor: colors.border }}>
        <div className="text-[8px] font-bold opacity-50 px-2 py-1 border-b" style={{ borderColor: colors.border }}>
        ...
        {pendingOrders.length === 0 ? (
          ...
          <table className="w-full text-[9px]">
            <thead>
              <tr className="text-left opacity-50" style={{ borderBottom: `1px solid ${colors.border}` }}>
                ...
                <tr key={o.id} className="border-b" style={{ borderColor: `${colors.border}44` }}>
                  ...
                  <td className="px-1">
                    <button onClick={() => cancelOrder(o.id)}
                      className="hover:opacity-80" style={{ color: "#FF4444" }}>
