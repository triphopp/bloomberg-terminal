...
import { Loader2, RefreshCw, AlertTriangle, Shield, TrendingDown, Activity, Clock, Infinity as InfinityIcon } from "lucide-react";
import {
  Bar, BarChart, Cell, ResponsiveContainer, Tooltip, XAxis, YAxis,
} from "recharts";
import { fmt, fmtK, pnlColor, type Colors } from "../helpers";
...
interface RiskMetrics {
  ...
  risk_score: number;
  assets: { symbol: string; weight_pct: number; risk_contribution_pct: number; volatility_annual: number; var_contribution: number }[];
  correlation_matrix: { symbols: string[]; matrix: number[][] };
  trim_signals: { symbol: string; action: string; reason: string; excess_rc_pct: number; suggested_trim_pct: number }[];
  // Breach checker
  today_return_pct: number;
  breach_hist: boolean;
  breach_cf: boolean;
  ...
  kupiec_pass: boolean;
  account_breakdown?: Record<string, {
    portfolio_value: number; var_parametric_pct: number; cvar_pct: number;
    volatility_annual_pct: number; max_drawdown_pct: number; sharpe_ratio: number;
    risk_score: number; n_positions: number;
  }>;
}
...
interface ParityData {
  ...
  rebalance_actions: { symbol: string; action: string; current_weight_pct: number; optimal_weight_pct: number; drift_pct: number; trade_value: number; shares_change?: number; current_price?: number }[];
  method: string;
  portfolio_value?: number;
}
...
interface GreeksRow {
  ...
  price_adj: number; delta_adj: number; gamma_adj: number; theta_adj: number; vega_adj: number; rho_adj: number;
  delta_diff: number; gamma_diff: number; theta_diff: number; vega_diff: number;
  T_years: number; days_to_exp: number; iv: number;
  skew_input: number; kurt_input: number;
  error?: string;
}

interface OptionsRiskPosition {
  id: string; underlying: string; expiry: string; strike: number;
  option_type: "call" | "put"; quantity: number; entry_price: number;
  max_loss: number | null; unlimited_loss: boolean;
  greeks: GreeksRow;
}
...
interface OptionsRiskData {
  ...
  portfolio: {
    ...
    expiry_alerts: { id: string; underlying: string; strike: number; option_type: string; expiry: string; days_to_exp: number; level: "critical" | "warn" }[];
  };
  freshness: { is_realtime: boolean; delay_minutes: number; warning: string };
}

export function RiskTab({ accountId, currency, colors }: {
  ...
  return (
    <div className="overflow-y-auto px-2 py-1" style={{ maxHeight: "calc(100vh - 220px)" }}>
        ...
        {SUB_TABS.map(t => (
          <button key={t.id}
            ...
            style={{
              ...
              borderBottom: subTab === t.id ? `2px solid ${colors.accent}` : "2px solid transparent",
            }}
            onClick={() => setSubTab(t.id)}>
            {t.label}
          </button>
        ))}
        <button onClick={loadMetrics} disabled={loading} className="ml-auto p-0.5">
          {loading ? <Loader2 className="h-3 w-3 animate-spin" style={{ color: colors.textSecondary }} />
      ...
      {metrics && subTab === "overview" && <OverviewSection metrics={metrics} colors={colors} sym={sym} riskColor={riskColor} accountId={accountId} />}
      {subTab === "options" && (
        <OptionsRiskSection data={optionsRisk} loading={loadingOpts} colors={colors} onRefresh={loadOptionsRisk} />
      )}
    </div>
  );
}
...
function EWSHistorySection({ accountId, colors }: { accountId: string; colors: Colors }) {
  ...
  useEffect(() => { if (open && history.length === 0) load(); }, [open, history.length, load]);
  ...
  const volColor = (r: string) =>
    r === "STRESSED" ? "#FF4444" : r === "ELEVATED" ? "#ff9900" : r === "CALM" ? "#00AA44" : "#444444";
  ...
  const SIGNALS = [
    { label: "EWS",      getValue: (s: RiskSnapshot) => ewsColor(s.ews),                getText: (s: RiskSnapshot) => String(s.ews) },
    { label: "Vol",      getValue: (s: RiskSnapshot) => volColor(s.vol_regime),          getText: (s: RiskSnapshot) => s.vol_regime[0] },
    { label: "Ensemble", getValue: (s: RiskSnapshot) => ensColor(s.ensemble_signal),     getText: (s: RiskSnapshot) => s.ensemble_signal === "STABLE" ? "S" : s.ensemble_signal === "FAT_TAIL_RISK" ? "F" : "C" },
    { label: "CF/Hist",  getValue: (s: RiskSnapshot) => cfColor(s.cf_hist_ratio),        getText: (s: RiskSnapshot) => s.cf_hist_ratio.toFixed(2) },
    { label: "Breach",   getValue: (s: RiskSnapshot) => breachColor(s.breach_count),     getText: (s: RiskSnapshot) => String(s.breach_count) },
    { label: "Drawdown", getValue: (s: RiskSnapshot) => ddColor(s.current_drawdown_pct), getText: (s: RiskSnapshot) => `${s.current_drawdown_pct.toFixed(1)}%` },
  ];

  const abbr = (d: string) => d.slice(5); // "MM-DD"

  return (
    <div className="rounded" style={{ border: `1px solid ${colors.border}`, background: "#0a0a0a" }}>
      <button className="w-full flex items-center gap-2 px-2 py-1.5" onClick={() => setOpen(o => !o)}>
            ...
            <>
              ...
              <div className="flex items-center gap-3 mb-2 flex-wrap">
                {[
                  ...
                  <div key={l.label} className="flex items-center gap-1">
                    ...
                    <span className="text-[7px]" style={{ color: "#666" }}>{l.label}</span>
                  </div>
                ))}
                <div className="flex items-center gap-1 ml-2">
                  <div className="w-2.5 h-2.5 rounded-sm" style={{ background: "#FF444433", border: "1px solid #FF4444" }} />
                  <span className="text-[7px]" style={{ color: "#FF6666" }}>Fat tail event</span>
                </div>
              </div>
              ...
              <div className="overflow-x-auto">
                <table style={{ borderCollapse: "separate", borderSpacing: "2px 2px" }}>
                  ...
                  <tbody>
                    {SIGNALS.map(sig => (
                      <tr key={sig.label}>
                        <td className="text-[7px] pr-1.5 text-right font-bold"
                    ...
                    <tr>
                      <td className="text-[7px] pr-1.5 text-right font-bold" style={{ color: "#555" }}>Return</td>
                      {history.map(s => {
              ...
              {fatDates.length > 0 && (
                <div className="mt-2 space-y-0.5">
                  {history.filter(s => s.is_fat_tail_event).map(s => (
                    <div key={s.snapshot_date}
                      ...
                      <span style={{ color: colors.textSecondary }}>
                        {s.vol_regime} · {s.ensemble_signal} · DD {s.current_drawdown_pct.toFixed(1)}%
                      </span>
              ...
              {fatDates.length > 0 && (() => {
                ...
                history.forEach((s, i) => {
                  if (!s.is_fat_tail_event && s.ews >= 5) {
                    ...
                    if (upcoming) {
                      ...
                      if (s.cf_hist_ratio >= 1.1) sigs.push(`CF/Hist ${s.cf_hist_ratio.toFixed(2)}`);
                ...
                return (
                  <div className="mt-2">
                    ...
                    {preWarnings.map(w => (
                      <div key={w.date} className="flex items-center gap-1.5 text-[7px] py-0.5"
                        style={{ color: colors.textSecondary }}>
                        <span style={{ color: "#ff9900" }}>{w.date}</span>
                        <span>EWS={w.ews}</span>
                        {w.signals.map(sg => (
                          <span key={sg} className="px-1 rounded"
                            style={{ background: "#1a1000", color: "#ff9900" }}>{sg}</span>
                        ))}
                      </div>
                    ))}
                  </div>
...
function VaRBreachChecker({ metrics, colors, sym }: {
  metrics: RiskMetrics; colors: Colors; sym: string;
}) {
  const { today_return_pct, breach_hist, breach_cf, breach_mc,
    kupiec_pvalue, kupiec_pass,
    var_historical_pct, var_cf_pct, cvar_mc_pct,
    var_backtest_exceptions, var_backtest_rate, lookback_days,
    var_backtest_signal, confidence } = metrics;
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="p-2 rounded" style={{ background: "#111", border: `1px solid ${colors.border}` }}>
        ...
        <div className="flex items-center gap-3">
          <span className="text-lg font-bold font-mono"
            ...
            {today_return_pct >= 0 ? "+" : ""}{today_return_pct.toFixed(3)}%
          </span>
          {anyBreach ? (
      ...
      <div className="p-2 rounded" style={{ background: "#111", border: `1px solid ${colors.border}` }}>
          ...
          <tbody>
            {methods.map(m => (
              <tr key={m.label} className="border-t" style={{ borderColor: colors.border }}>
                <td className="py-1 font-bold" style={{ color: colors.text }}>{m.label}</td>
                ...
                <td className="py-1 text-right text-[7px]" style={{ color: "#555" }}>{m.note}</td>
                <td className="py-1 text-right">
                  {m.breached ? (
                    <span className="font-bold" style={{ color: "#FF4444" }}>✗ EXCEEDED</span>
                  ) : (
        ...
        <div className="mt-2 pt-2 border-t" style={{ borderColor: colors.border }}>
          ...
          {anyBreach && allBreach && (
            <div className="p-1.5 rounded" style={{ background: "#2a0000", border: "1px solid #FF4444" }}>
          ...
          {anyBreach && !allBreach && (
            <div className="p-1.5 rounded" style={{ background: "#1a1000", border: "1px solid #ff9900" }}>
              ...
              <div className="text-[8px] mt-0.5 space-y-0.5">
                ...
                {methods.filter(m => m.breached).map(m => (
          ...
          <>
            <div className="grid grid-cols-3 gap-2 mb-2">
              <div className="p-1.5 rounded text-center" style={{ background: "#0a0a0a" }}>
                <div className="text-[7px]" style={{ color: colors.textSecondary }}>Exceptions</div>
                <div className="text-[11px] font-bold font-mono" style={{ color: colors.text }}>
                  {var_backtest_exceptions}/{lookback_days}
                </div>
              </div>
              <div className="p-1.5 rounded text-center" style={{ background: "#0a0a0a" }}>
                <div className="text-[7px]" style={{ color: colors.textSecondary }}>Exception rate</div>
                <div className="text-[11px] font-bold font-mono"
                  style={{ color: var_backtest_rate > parseFloat(expectedRate) ? "#FF4444" : "#00FF00" }}>
              ...
              <div className="p-1.5 rounded text-center" style={{ background: "#0a0a0a" }}>
                <div className="text-[7px]" style={{ color: colors.textSecondary }}>Kupiec p-value</div>
            ...
            {!kupiec_pass && (
              <div className="mt-1.5 text-[7px] space-y-0.5" style={{ color: colors.textSecondary }}>
      ...
      <div className="text-[7px] px-1" style={{ color: "#444" }}>
        "Most recent session" = last trading day in yfinance data (~15–30min delayed).
        Kupiec POF test uses {lookback_days}d historical exception count vs binomial null H₀.
      </div>
    </div>
  );
}
...
function EnsembleSignalBadge({ signal }: { signal: RiskMetrics["ensemble_signal"] }) {
  const cfg = {
    ...
    CORRELATION_RISK:  { label: "CORR RISK ⚠", bg: "#1a0000", border: "#FF444477", color: "#FF4444" },
  }[signal];
  return (
...
function BacktestBadge({ signal, exceptions, rate, lookback }: {
  signal: RiskMetrics["var_backtest_signal"]; exceptions: number; rate: number; lookback: number;
}) {
...
function EnsembleRow({ label, pct, amt, note, color, sym }: {
  label: string; pct: number; amt: number; note: string; color: string; sym: string;
}) {
  return (
    <tr>
      <td className="py-0.5 font-bold" style={{ color }}>{label}</td>
      <td className="text-right py-0.5 font-mono" style={{ color }}>{pct.toFixed(2)}%</td>
      <td className="text-right py-0.5 font-mono text-[7px]" style={{ color: "#888" }}>{sym}{fmtK(amt)}</td>
      <td className="text-right py-0.5 text-[7px]" style={{ color: "#666" }}>{note}</td>
    </tr>
  );
}

function RegimeBadge({ regime, stressedPct, stressedAmt, sym }: {
  regime: RiskMetrics["vol_regime"]; stressedPct: number; stressedAmt: number; sym: string;
}) {
  const cfg = {
    ...
    <span style={{ color: cfg.color }}>
      ...
      {(regime === "STRESSED" || regime === "ELEVATED") && stressedPct > 0 && (
        <span style={{ color: "#FF4444", marginLeft: 4 }}>
          · Stressed CVaR: {stressedPct.toFixed(2)}% ({sym}{fmtK(stressedAmt)})
        </span>
      )}
    </span>
  );
...
function OverviewSection({ metrics, colors, sym, riskColor, accountId }: {
  metrics: RiskMetrics; colors: Colors; sym: string;
  riskColor: (s: number) => string; accountId: string;
}) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/v2/portfolio/risk/risk-parity?lookback=252${qs}`)
      .then(r => r.json()).then(d => setParity(d)).catch(() => {})
      .finally(() => setParityLoading(false));
  }, [chartView, parity, parityLoading, accountId]);
  const scale = Math.sqrt(varHorizon.days);
  const breachCount = [metrics.breach_hist, metrics.breach_cf, metrics.breach_mc].filter(Boolean).length;
    ...
    <div className="space-y-1.5">
      ...
      <div className="flex items-center gap-2 px-2 py-1 rounded" style={{ background: "#111" }}>
        ...
        <div className="flex items-baseline gap-1 shrink-0">
          ...
          <span className="text-[6px]" style={{ color: colors.textSecondary }}>RISK</span>
        </div>
        <div className="w-px h-4 shrink-0" style={{ background: colors.border }} />
        {/* Vol regime */}
        {metrics.vol_regime !== "UNKNOWN" && (
          <div className="text-[7px] shrink-0">
            <RegimeBadge regime={metrics.vol_regime}
              stressedPct={metrics.cvar_stressed_pct * scale} sym={sym}
        ...
        <span className="text-[7px] font-mono shrink-0" style={{ color: colors.textSecondary }}>
          σ {metrics.volatility_annual_pct.toFixed(1)}%ann · {metrics.volatility_daily_pct.toFixed(3)}%d
        </span>
        {/* Horizon selector */}
        <div className="flex items-center gap-0 ml-auto shrink-0">
          <span className="text-[6px] mr-1" style={{ color: colors.textSecondary }}>HRZ</span>
          {VAR_HORIZONS.map(h => (
            <button key={h.label} className="text-[7px] px-1.5 py-0.5 font-bold"
              style={{
                color: varHorizon.label === h.label ? colors.accent : colors.textSecondary,
                borderBottom: varHorizon.label === h.label ? `1px solid ${colors.accent}` : "1px solid transparent",
              }}
              onClick={() => setVarHorizon(h)}>{h.label}</button>
          ))}
        </div>
        <div className="w-px h-4 shrink-0" style={{ background: colors.border }} />
        {/* Today return */}
        <div className="flex items-center gap-1 shrink-0">
          <span className="text-[6px]" style={{ color: colors.textSecondary }}>TODAY</span>
          <span className="text-[8px] font-mono font-bold"
            style={{ color: metrics.today_return_pct >= 0 ? "#00FF00" : "#FF4444" }}>
            {metrics.today_return_pct >= 0 ? "+" : ""}{metrics.today_return_pct.toFixed(2)}%
          </span>
        </div>
        {/* Trim chips */}
          ...
          <>
            ...
            <div className="flex items-center gap-0.5 flex-wrap">
              ...
              {metrics.trim_signals.map((s, i) => (
                <span key={i} className="text-[6px] px-1 py-0.5 rounded font-mono"
                  style={{ background: "#2a0000", border: "1px solid #FF444433", color: "#FF6666" }}
                  title={s.reason}>
                  {s.symbol} −{s.suggested_trim_pct}%
                </span>
              ))}
            </div>
          </>
      ...
      <div className="grid grid-cols-9 gap-px">
        {([
          { label: "SHARPE",  value: metrics.sharpe_ratio.toFixed(2),               good: metrics.sharpe_ratio > 1 },
          { label: "SORTINO", value: metrics.sortino_ratio.toFixed(2),              good: metrics.sortino_ratio > 1.5 },
          { label: "CALMAR",  value: metrics.calmar_ratio.toFixed(2),               good: metrics.calmar_ratio > 1 },
          { label: "MAX DD",  value: `${metrics.max_drawdown_pct.toFixed(1)}%`,     good: metrics.max_drawdown_pct < 15 },
          { label: "CUR DD",  value: `${metrics.current_drawdown_pct.toFixed(1)}%`, good: metrics.current_drawdown_pct < 5 },
          { label: "DIV",     value: metrics.diversification_ratio.toFixed(2),      good: metrics.diversification_ratio > 1.5 },
          { label: "EFF N",   value: metrics.effective_n.toFixed(1),                good: metrics.effective_n > 3 },
          { label: "BREACH",  value: `${breachCount}/3`,                            good: breachCount === 0 },
          { label: "KUPIEC",  value: metrics.kupiec_pass ? "PASS" : "FAIL",         good: metrics.kupiec_pass },
        ] as const).map(({ label, value, good }) => (
          <div key={label} className="flex flex-col items-center justify-center py-1 rounded"
            style={{ background: "#111" }}>
            <span className="text-[6px]" style={{ color: colors.textSecondary }}>{label}</span>
            <span className="text-[8px] font-mono font-bold" style={{ color: good ? "#00FF00" : "#FF4444" }}>{value}</span>
          </div>
        ))}
      </div>
        ...
        <div className="col-span-2 space-y-1.5">
          <div className="p-2 rounded" style={{ background: "#111", border: `1px solid ${colors.border}` }}>
            <div className="flex items-center gap-1 mb-1">
              ...
              <span className="text-[7px]" style={{ color: colors.textSecondary }}>ENSEMBLE VaR/CVaR 95%</span>
              <EnsembleSignalBadge signal={metrics.ensemble_signal} />
              <BacktestBadge signal={metrics.var_backtest_signal} exceptions={metrics.var_backtest_exceptions}
                rate={metrics.var_backtest_rate} lookback={metrics.lookback_days} />
            </div>
            <table className="w-full text-[7px]">
              ...
              <tbody>
                {([
                  { label: "Hist CVaR", pct: metrics.cvar_pct * scale,    amt: metrics.cvar_amount * scale,    color: "#ff9900",                                                                                   breach: metrics.breach_hist },
                  { label: "CF VaR",   pct: metrics.var_cf_pct * scale,   amt: metrics.var_cf_amount * scale,   color: metrics.ensemble_signal === "FAT_TAIL_RISK" ? "#FF4444" : colors.text,                      breach: metrics.breach_cf },
                  { label: "MC CVaR",  pct: metrics.cvar_mc_pct * scale,  amt: metrics.cvar_mc_amount * scale,  color: metrics.ensemble_signal === "CORRELATION_RISK" ? "#FF4444" : colors.text,                   breach: metrics.breach_mc },
                ] as const).map(row => (
                  <tr key={row.label}>
                    <td style={{ color: colors.textSecondary }}>{row.label}</td>
                    <td className="text-right font-mono" style={{ color: row.color }}>{row.pct.toFixed(2)}%</td>
                    <td className="text-right font-mono text-[6px]" style={{ color: colors.textSecondary }}>{sym}{fmtK(row.amt)}</td>
                    <td className="text-right" style={{ color: row.breach ? "#FF4444" : "#444444" }}>●</td>
                  </tr>
                ))}
              </tbody>
            </table>
            <div className="mt-1 pt-1 border-t flex items-baseline justify-between" style={{ borderColor: colors.border }}>
              <span className="text-[6px]" style={{ color: colors.textSecondary }}>Conservative</span>
              ...
              <span className="text-[6px]" style={{ color: colors.textSecondary }}>
                {sym}{fmtK(metrics.ensemble_conservative_amount * scale)}
              </span>
            </div>
            {metrics.cvar_ci_lo > 0 && (
              <div className="text-[6px] mt-0.5" style={{ color: metrics.cvar_ci_width_ratio > 1.5 ? "#ff9900" : "#555" }}>
                CI {(metrics.cvar_ci_lo * scale).toFixed(2)}%–{(metrics.cvar_ci_hi * scale).toFixed(2)}%
          ...
          <div className="px-2 py-1.5 rounded text-[7px] space-y-0.5" style={{ background: "#111" }}>
            <div className="flex items-center justify-between">
              <span style={{ color: colors.textSecondary }}>Backtest rate</span>
              <span className="font-mono" style={{ color: metrics.var_backtest_signal === "GREEN" ? "#00FF00" : metrics.var_backtest_signal === "YELLOW" ? "#ff9900" : "#FF4444" }}>
                {(metrics.var_backtest_rate * 100).toFixed(1)}%
              </span>
              <span style={{ color: colors.textSecondary }}>({metrics.var_backtest_exceptions} exc / {metrics.lookback_days}d)</span>
            </div>
            <div className="flex items-center justify-between">
              <span style={{ color: colors.textSecondary }}>Kupiec p-val</span>
              <span className="font-mono" style={{ color: metrics.kupiec_pass ? "#00FF00" : "#FF4444" }}>
                ...
                {(["contrib", "parity"] as const).map(v => (
                  <button key={v} onClick={() => setChartView(v)}
                    ...
                    style={{
                      ...
                      borderBottom: chartView === v ? `1px solid ${colors.accent}` : "1px solid transparent",
                    }}>
              ...
              {chartView === "contrib" && (
                <ResponsiveContainer width="100%" height={Math.max(70, metrics.assets.length * 14)}>
                  <BarChart data={metrics.assets} layout="vertical" margin={{ left: 56, right: 6 }}>
                    ...
                    <YAxis type="category" dataKey="symbol" tick={{ fontSize: 7, fill: colors.text }} width={54} interval={0} />
                    <Tooltip contentStyle={{ background: "#111", border: `1px solid ${colors.border}`, fontSize: 8 }} />
                    ...
                    <Bar dataKey="risk_contribution_pct" name="Risk Contrib %" barSize={5}>
                      {metrics.assets.map((a, i) => (
                        <Cell key={i} fill={a.risk_contribution_pct > a.weight_pct * 1.5 ? "#FF4444" : "#ff9900"} />
                      ))}
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
                ...
                <>
                  ...
                  {!parityLoading && parity && parity.current_weights.length > 0 && (() => {
                    const combined = parity.current_weights.map((c, i) => ({
                      symbol: c.symbol, current: c.weight_pct,
                      optimal: parity.optimal_weights[i]?.weight_pct ?? 0,
                    }));
                    return (
                      <>
                        <ResponsiveContainer width="100%" height={Math.max(70, combined.length * 14)}>
                          <BarChart data={combined} layout="vertical" margin={{ left: 56, right: 6 }}>
                            <XAxis type="number" tick={{ fontSize: 7, fill: colors.textSecondary }} unit="%" />
                            <YAxis type="category" dataKey="symbol" tick={{ fontSize: 7, fill: colors.text }} width={54} interval={0} />
                            <Tooltip contentStyle={{ background: "#111", border: `1px solid ${colors.border}`, fontSize: 8 }} />
                        ...
                        {parity.rebalance_actions.length > 0 ? (
                          <div className="mt-1 space-y-0.5">
                            {parity.rebalance_actions.map((a, i) => (
                              <div key={i} className="flex items-center gap-1 text-[7px] px-1 py-0.5 rounded"
                                style={{ background: "#111" }}>
                                <span className="w-7 font-bold" style={{ color: a.action === "BUY" ? "#00FF00" : "#FF4444" }}>{a.action}</span>
                                <span className="font-bold" style={{ color: colors.text }}>{a.symbol}</span>
                                <span style={{ color: colors.textSecondary }}>
                                  {a.current_weight_pct.toFixed(1)}%→{a.optimal_weight_pct.toFixed(1)}%
                                </span>
                                <span className="ml-auto font-mono" style={{ color: a.drift_pct > 0 ? "#00FF00" : "#FF4444" }}>
                          ...
                          <div className="flex items-center gap-1 text-[7px] mt-1" style={{ color: "#00FF00" }}>
                  ...
                  {!parityLoading && (!parity || parity.current_weights.length === 0) && (
                    <div className="text-[7px] py-4 text-center" style={{ color: colors.textSecondary }}>
      ...
      {accountId === "all" && metrics.account_breakdown && (
        <div className="rounded" style={{ border: `1px solid ${colors.border}` }}>
          <button className="w-full flex items-center gap-2 px-2 py-1"
            onClick={() => setAcctOpen(o => !o)}>
            <span className="text-[8px] font-bold" style={{ color: colors.textSecondary }}>ACCOUNT BREAKDOWN</span>
            <span className="ml-auto text-[8px]" style={{ color: colors.textSecondary }}>{acctOpen ? "▾" : "▸"}</span>
          </button>
          {acctOpen && (
            <div className="grid grid-cols-3 gap-1 px-2 pb-2">
              {Object.entries(metrics.account_breakdown).map(([aid, m]) => (
                <div key={aid} className="p-1.5 rounded" style={{ background: "#111" }}>
                  <div className="text-[8px] font-bold" style={{ color: colors.accent }}>{aid.toUpperCase()}</div>
                  <div className="text-[7px] mt-0.5 space-y-0.5" style={{ color: colors.textSecondary }}>
                    <div>VaR {m.var_parametric_pct.toFixed(2)}% · Vol {m.volatility_annual_pct.toFixed(1)}%</div>
                    <div>Sharpe {m.sharpe_ratio.toFixed(2)} · DD {m.max_drawdown_pct.toFixed(1)}%</div>
                    <div style={{ color: riskColor(m.risk_score) }}>Score {m.risk_score.toFixed(0)} · N={m.n_positions}</div>
                  </div>
                </div>
              ))}
            </div>
...
function VaRBreachCompact({ metrics, colors }: { metrics: RiskMetrics; colors: Colors }) {
  const {
    today_return_pct, breach_hist, breach_cf, breach_mc,
    kupiec_pvalue, kupiec_pass,
    var_backtest_exceptions, var_backtest_rate, lookback_days, var_backtest_signal,
    confidence,
  } = metrics;
  ...
  return (
    <div className="p-2 rounded flex flex-col gap-1.5" style={{ background: "#111", border: `1px solid ${colors.border}` }}>
      <div className="text-[7px] font-bold" style={{ color: colors.textSecondary }}>VaR BREACH CHECK</div>

      {/* Today's return + breach badge */}
      <div className="flex items-center gap-1.5">
        <span className="text-[12px] font-bold font-mono"
          style={{ color: today_return_pct >= 0 ? "#00FF00" : "#FF4444" }}>
          {today_return_pct >= 0 ? "+" : ""}{today_return_pct.toFixed(2)}%
        </span>
        {anyBreach ? (
          <span className="text-[7px] px-1 py-0.5 rounded font-bold"
            style={{
      ...
      {var_backtest_signal !== "INSUFFICIENT_DATA" && (
        <div className="pt-1 border-t space-y-0.5" style={{ borderColor: colors.border }}>
          ...
          <div className="text-[7px]" style={{ color: colors.textSecondary }}>
            {var_backtest_exceptions}/{lookback_days}d · {var_backtest_rate.toFixed(1)}% exc (exp {expectedRate}%)
          </div>
          {!kupiec_pass && (
            <div className="text-[7px] font-bold" style={{ color: "#FF4444" }}>→ recalibrate lookback</div>
          )}
          {anyBreach && allBreach && (
            <div className="text-[7px] font-bold" style={{ color: "#FF4444" }}>→ reduce exposure 50%</div>
          )}
          {anyBreach && !allBreach && (
            <div className="text-[7px]" style={{ color: "#ff9900" }}>
              → review {[breach_hist && "Hist", breach_cf && "CF", breach_mc && "MC"].filter(Boolean).join(", ")}
            </div>
          )}
        </div>
      )}
...
function CorrelationSection({ metrics, colors }: { metrics: RiskMetrics; colors: Colors }) {
  ...
  for (let i = 0; i < n; i++)
    for (let j = i + 1; j < n; j++)
      offDiag.push(matrix[i][j]);

  const avgCorr = offDiag.length > 0
  ...
  for (let i = 0; i < n; i++)
    for (let j = i + 1; j < n; j++)
      if (matrix[i][j] >= 0.7)
        highPairs.push({ a: symbols[i], b: symbols[j], v: matrix[i][j] });
  ...
  return (
    <div className="rounded" style={{ border: `1px solid ${colors.border}`, background: "#0c0c0c" }}>
      ...
      >
        ...
        <span className="text-[7px] px-1 rounded" style={{ background: "#1a1a1a", color: colors.textSecondary }}>
          Ledoit-Wolf
        </span>
        {/* Summary stats */}
        ...
        <div className="px-2 pb-2">
          ...
          {highPairs.length > 0 && (
            <div className="mb-2 flex flex-wrap gap-1">
              {highPairs.map((p, i) => (
                <span key={i} className="text-[7px] px-1.5 py-0.5 rounded font-mono font-bold"
                  style={{
            ...
            <table style={{ borderCollapse: "separate", borderSpacing: 2 }}>
              <thead>
                <tr>
                  <th style={{ width: 52 }} />
                  {symbols.map(s => (
                    <th key={s} className="text-center text-[7px] font-bold pb-1"
              ...
              <tbody>
                {matrix.map((row, i) => (
                  <tr key={i}>
                    <td className="text-[7px] font-bold pr-1.5 text-right"
                      style={{ color: colors.text, maxWidth: 52, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                      {abbr(symbols[i])}
                    </td>
                    {row.map((v, j) => (
                      <td key={j}
          ...
          <div className="flex items-center gap-2 mt-2 flex-wrap">
            {[
              ...
              <div key={l.label} className="flex items-center gap-1">
                <div className="w-3 h-3 rounded-sm" style={{ background: l.bg, border: `1px solid ${l.fg}44` }} />
                <span className="text-[7px]" style={{ color: l.fg }}>{l.label}</span>
              </div>
            ))}
          </div>
        </div>
...
function ParitySection({ parity, colors, sym }: { parity: ParityData | null; colors: Colors; sym: string }) {
  if (!parity) return <Loader2 className="h-4 w-4 animate-spin mx-auto mt-4" style={{ color: colors.accent }} />;

  if (!parity.current_weights.length) {
    return <div className="text-[9px] p-4" style={{ color: colors.textSecondary }}>Need at least 2 positions with price history</div>;
  }
    ...
    <div>
      ...
      <ResponsiveContainer width="100%" height={Math.max(120, combined.length * 24)}>
        <BarChart data={combined} layout="vertical" margin={{ left: 60, right: 10 }}>
          ...
          <YAxis type="category" dataKey="symbol" tick={{ fontSize: 8, fill: colors.text }} width={58} interval={0} />
      ...
      {parity.rebalance_actions.length > 0 && (
        <div className="mt-2">
          <h3 className="text-[9px] font-bold mb-1" style={{ color: colors.textSecondary }}>REBALANCE ACTIONS (drift &gt; 3%)</h3>
          <div className="space-y-0.5">
            {parity.rebalance_actions.map((a, i) => (
              <div key={i} className="text-[8px] p-1.5 rounded" style={{ background: "#111" }}>
                <div className="flex items-center">
                  <span className="w-10 font-bold" style={{ color: a.action === "BUY" ? "#00FF00" : "#FF4444" }}>
                    {a.action}
                  </span>
                  <span className="w-16 font-bold" style={{ color: colors.text }}>{a.symbol}</span>
                  <span style={{ color: colors.textSecondary }}>
                    {a.current_weight_pct.toFixed(1)}% → {a.optimal_weight_pct.toFixed(1)}%
                  </span>
                  <span className="ml-auto font-bold" style={{ color: a.drift_pct > 0 ? "#00FF00" : "#FF4444" }}>
                    {a.drift_pct > 0 ? "+" : ""}{a.drift_pct.toFixed(1)}%
                  </span>
                  <span className="ml-2" style={{ color: colors.textSecondary }}>
                    {sym}{fmtK(Math.abs(a.trade_value))}
                  </span>
                </div>
                <div className="flex items-center gap-2 mt-0.5 pl-10">
                  {a.shares_change != null && a.shares_change !== 0 ? (
                    <span className="font-mono" style={{ color: a.action === "BUY" ? "#4ade80" : "#f87171" }}>
                      {a.action === "BUY" ? "+" : ""}{a.shares_change > 0 ? "+" : ""}{Math.abs(a.shares_change).toFixed(2)} shares
                    </span>
                  ) : a.current_price != null && a.current_price > 0 ? (
                    <span className="font-mono" style={{ color: a.action === "BUY" ? "#4ade80" : "#f87171" }}>
                      {a.action === "BUY" ? "+" : "−"}{Math.abs(a.trade_value / a.current_price).toFixed(2)} shares
                    </span>
                  ) : null}
                  {a.current_price != null && (
                    <span style={{ color: colors.textSecondary }}>@ {sym}{fmt(a.current_price)}</span>
                  )}
                </div>
              </div>
            ))}
      ...
      {parity.rebalance_actions.length === 0 && (
        <div className="mt-2 flex items-center gap-1 text-[9px] p-2 rounded" style={{ background: "#001a00", border: "1px solid #00FF0044" }}>
          <Shield className="h-3 w-3" style={{ color: "#00FF00" }} />
          <span style={{ color: "#00FF00" }}>Portfolio is within risk parity tolerance (drift &lt; 3%)</span>
        </div>
      )}
    </div>
  );
...
function MetricCard({ label, value, sub, colors, icon }: {
  label: string; value: string; sub: string; colors: Colors; icon: React.ReactNode;
}) {
  return (
    <div className="p-2 rounded" style={{ background: "#111", border: `1px solid ${colors.border}` }}>
      <div className="flex items-center gap-1 mb-0.5">
        <span style={{ color: colors.textSecondary }}>{icon}</span>
        <span className="text-[8px]" style={{ color: colors.textSecondary }}>{label}</span>
      </div>
      <div className="text-[10px] font-bold" style={{ color: colors.text }}>{value}</div>
      <div className="text-[8px]" style={{ color: colors.textSecondary }}>{sub}</div>
    </div>
  );
}

function MiniMetric({ label, value, good, colors }: {
  label: string; value: string; good: boolean; colors: Colors;
}) {
  return (
    <div className="text-center p-1 rounded" style={{ background: "#111" }}>
      <div className="text-[8px]" style={{ color: colors.textSecondary }}>{label}</div>
      <div className="text-[9px] font-bold" style={{ color: good ? "#00FF00" : "#ff9900" }}>{value}</div>
    </div>
  );
}
...
function DiffCell({ diff, d = 4 }: { diff: number | undefined; d?: number }) {
  if (diff == null) return <td className="px-1 py-1 text-right text-[8px]" style={{ color: "#555" }}>—</td>;
  ...
  return (
    <div className="space-y-3">
        ...
        <div className="space-y-1">
          {portfolio.expiry_alerts.map(a => (
            <div key={a.id}
              ...
              <span className="font-bold">{a.underlying} {a.strike}{a.option_type === "call" ? "C" : "P"}</span>
      ...
      {portfolio.has_short_positions && (
        <div className="flex items-center gap-2 px-2 py-1.5 rounded text-[9px] border"
          style={{ background: "#FF444411", borderColor: "#FF4444", color: "#FF4444" }}>
          <AlertTriangle className="h-3 w-3 flex-shrink-0" />
          <span className="font-bold">SHORT positions detected — potential unlimited loss on short calls</span>
        </div>
      )}
      ...
      <div className="grid grid-cols-3 gap-2">
        <div className="p-2 rounded" style={{ background: "#111", border: `1px solid ${colors.border}` }}>
          <div className="text-[8px] mb-1" style={{ color: colors.textSecondary }}>PREMIUM AT RISK</div>
          ...
          <div className="text-[8px]" style={{ color: colors.textSecondary }}>max loss (long only)</div>
        </div>
        <div className="p-2 rounded" style={{ background: "#111", border: `1px solid ${colors.border}` }}>
          <div className="text-[8px] mb-1" style={{ color: colors.textSecondary }}>DAILY THETA BLEED (Adj)</div>
          <div className="text-[12px] font-bold font-mono" style={{ color: thetaColor }}>
            {portfolio.total_theta_adj_day >= 0 ? "+" : ""}${Math.abs(portfolio.total_theta_adj_day).toFixed(2)}
          </div>
          <div className="text-[8px]" style={{ color: colors.textSecondary }}>
            BS: {portfolio.total_theta_day >= 0 ? "+" : ""}${Math.abs(portfolio.total_theta_day).toFixed(2)}/day
          </div>
        </div>
        <div className="p-2 rounded" style={{ background: "#111", border: `1px solid ${colors.border}` }}>
          <div className="text-[8px] mb-1" style={{ color: colors.textSecondary }}>POSITIONS</div>
          <div className="text-[12px] font-bold font-mono" style={{ color: colors.text }}>{positions.length}</div>
          <div className="text-[8px]" style={{ color: colors.textSecondary }}>
      ...
      {Object.keys(portfolio.net_delta_by_underlying).length > 0 && (
        <div>
          <div className="text-[9px] font-bold mb-1" style={{ color: colors.accent }}>NET DELTA (shares equivalent)</div>
            ...
            <tbody>
              {Object.entries(portfolio.net_delta_by_underlying).map(([sym, d]) => (
                <tr key={sym} className="border-b" style={{ borderColor: colors.border }}>
                  <td className="px-1 py-1 font-bold" style={{ color: colors.text }}>{sym}</td>
                  <td className="px-1 py-1 text-right" style={{ color: d.bs >= 0 ? "#00FF00" : "#FF4444" }}>
                    {d.bs >= 0 ? "+" : ""}{d.bs.toFixed(1)}
                  </td>
                  <td className="px-1 py-1 text-right font-bold" style={{ color: d.adj >= 0 ? "#00FF00" : "#FF4444" }}>
                    {d.adj >= 0 ? "+" : ""}{d.adj.toFixed(1)}
                  </td>
      ...
      <div>
        ...
        <table className="w-full text-[9px] font-mono">
          <thead>
            <tr style={{ color: colors.textSecondary }}>
              {["Contract", "Qty", "DTE", "IV%", "Δ adj", "Γ adj", "Θ adj/day", "V adj", "Max loss"].map(h => (
                <th key={h} className={`px-1 py-0.5 ${h === "Contract" ? "text-left" : "text-right"}`}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
                ...
                <React.Fragment key={p.id}>
                  ...
                  >
                    <td className="px-1 py-1">
                      <span className="font-bold" style={{ color: colors.text }}>{p.underlying}</span>
                      <span className="text-[8px] ml-1 px-0.5 rounded" style={{ color: typeColor, border: `1px solid ${typeColor}` }}>
                        {p.option_type === "call" ? "C" : "P"}
                      </span>
                      <span className="text-[8px] ml-1" style={{ color: colors.textSecondary }}>{p.strike}</span>
                    </td>
                    <td className="px-1 py-1 text-right" style={{ color: p.quantity > 0 ? "#00FF00" : "#FF4444" }}>
                      {p.quantity > 0 ? "+" : ""}{p.quantity}
                    </td>
                    <td className="px-1 py-1 text-right"
                      style={{ color: hasGreeks && g.days_to_exp <= 7 ? "#FF4444" : colors.textSecondary }}>
                    ...
                    <td className="px-1 py-1 text-right"
                      ...
                      {hasGreeks ? `$${(g.theta_adj * Math.abs(p.quantity) * 100).toFixed(2)}` : "—"}
                    </td>
                    <td className="px-1 py-1 text-right" style={{ color: colors.text }}>
                    ...
                    <td className="px-1 py-1 text-right">
                      {p.unlimited_loss
                        ? <span style={{ color: "#FF4444" }} title="Unlimited — short call">∞</span>
                        : p.max_loss != null
                          ? <span style={{ color: "#FF4444" }}>${p.max_loss.toLocaleString("en-US", { maximumFractionDigits: 0 })}</span>
                          : "—"}
                    </td>
                  </tr>
                    ...
                    <tr style={{ background: "#0a0a0a" }}>
                      <td colSpan={9} className="px-3 py-2">
                        <div className="text-[8px] mb-1 font-bold" style={{ color: colors.accent }}>
                          {p.underlying} {p.strike}{p.option_type === "call" ? "C" : "P"}
                          {" · "}skew={g.skew_input} · kurt={g.kurt_input} · T={g.T_years.toFixed(3)}yr
                        </div>
                        <table className="text-[8px] font-mono w-auto">
                          ...
                          <tbody>
                            {([
                              ["Delta", g.delta,  g.delta_adj,  g.delta_diff,  "directional per share"],
                              ["Gamma", g.gamma,  g.gamma_adj,  g.gamma_diff,  "delta change per $1 move"],
                              ["Theta", g.theta,  g.theta_adj,  g.theta_diff,  "daily time decay (per contract)"],
                              ["Vega",  g.vega,   g.vega_adj,   g.vega_diff,   "per 1pp IV change"],
                              ["Rho",   g.rho,    g.rho_adj,    undefined,     "per 1pp rate change"],
                            ] as [string, number, number, number | undefined, string][]).map(([name, bs, adj, diff, note]) => (
                              <tr key={name}>
                                <td className="px-2 py-0.5 font-bold" style={{ color: colors.text }}>{name}</td>
                                <td className="px-2 py-0.5 text-right" style={{ color: colors.textSecondary }}>{bs.toFixed(4)}</td>
                                <td className="px-2 py-0.5 text-right font-bold" style={{ color: colors.text }}>{adj.toFixed(4)}</td>
                                <DiffCell diff={diff} d={4} />
                                <td className="px-2 py-0.5 pl-3" style={{ color: colors.textSecondary }}>{note}</td>
                              </tr>
                            ))}
                          </tbody>
                        </table>
      ...
      <div className="flex items-center gap-1 text-[8px]" style={{ color: "#f59e0b" }}>
        <Clock className="h-2.5 w-2.5" />
        ~{data.freshness.delay_minutes}m delayed · Greeks cached 5min · {data.freshness.warning}
