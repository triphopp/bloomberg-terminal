...
export function PortfolioView() {
  ...
  const createAccount = useCallback(async () => {
    ...
    if (!id) { setNewError("Invalid name"); return; }
    setNewError("");
    try {
      ...
      if (!r.ok) { const d = await r.json().catch(() => ({})); setNewError(d.detail || "Create failed"); return; }
      setNewName(""); setNewCurrency("THB"); setNewCountry("TH");
  ...
  const confirmDelete = useCallback(async () => {
    ...
    try {
      ...
      if (!r.ok) { setDeleteError(d.detail || "Delete failed"); setDeleting(false); return; }
    ...
    <div className="flex flex-col h-full" style={{ background: "#000", color: colors.text }}>
      ...
      <div className="flex items-center gap-px px-2 pt-1.5 border-b overflow-x-auto" style={{ borderColor: colors.border, background: "#080808" }}>
        ...
        {accounts.map(acc => (
          <div key={acc.id} className="group relative flex items-center">
            ...
            <button
              ...
              onClick={(e) => { e.stopPropagation(); openDeleteModal(acc); }}>
              ×
            </button>
          </div>
        ))}
        ...
        <button className={acctBtnCls}
          ...
          onClick={() => { setShowNewForm(s => !s); setNewError(""); }}>
          + NEW
        </button>
      ...
      {/* New-account form */}
      {showNewForm && (
        <div className="flex flex-wrap items-center gap-2 px-3 py-1.5 border-b text-[9px]"
          ...
          <button onClick={createAccount}
            ...
            style={{ borderColor: colors.accent, color: colors.accent, background: `${colors.accent}22` }}>
            CREATE
          </button>
      ...
      {deleteTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center"
          ...
          <div className="flex flex-col gap-4 p-6 border w-[400px] max-w-[90vw]"
            style={{ background: "#0d0d0d", borderColor: "#FF4444", color: colors.text }}>

            {/* Header */}
            <div className="flex items-center gap-2">
              <span className="text-[10px] font-bold" style={{ color: "#FF4444" }}>⚠ DELETE ACCOUNT</span>
            </div>
            ...
            <div className="flex flex-col gap-1.5">
              <label className="text-[8px] font-bold uppercase tracking-widest" style={{ color: colors.textSecondary }}>
              ...
              />
              {deleteError && (
                <span className="text-[8px]" style={{ color: "#FF4444" }}>{deleteError}</span>
              )}
            </div>
            ...
            <div className="flex gap-2 justify-end">
              ...
              <button
                ...
                style={{
                  borderColor: deleteConfirm === deleteTarget.name && !deleting ? "#FF4444" : "#333",
                  color: deleteConfirm === deleteTarget.name && !deleting ? "#FF4444" : "#444",
                  background: deleteConfirm === deleteTarget.name && !deleting ? "rgba(255,68,68,0.12)" : "transparent",
                  cursor: deleteConfirm === deleteTarget.name && !deleting ? "pointer" : "not-allowed",
                }}>
