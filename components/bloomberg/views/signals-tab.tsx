...
function RiskCardCompact({ colors }: { colors: Colors }) {
  ...
  if (isLoading || !data) {
    return (
      ...
      >
        <div
          className="text-[9px] font-mono font-bold tracking-widest mb-2"
      ...
      <div className="flex items-center justify-between">
        ...
        <div
          ...
          style={{
            ...
            border: `1px solid ${trafficClr}40`,
          }}
        >
        ...
        {Object.entries(indicators).map(([key, ind]) => {
          ...
          return (
            <div key={key} className="flex items-center gap-2 text-[9px] font-mono">
              <span className="w-14 shrink-0" style={{ color: colors.textSecondary }}>
                {label}
              </span>
              ...
              <span className="w-6 text-right shrink-0" style={{ color: scoreColor }}>
                {ind.score.toFixed(0)}
              </span>
            </div>
          );
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-3 text-[9px] font-mono">
        ...
        <span
          className="font-bold"
          style={{ color: wScore > 0 ? "#00e676" : wScore < 0 ? "#ff1744" : "#9e9e9e" }}
        >
          {wScore > 0 ? "+" : ""}
          {wScore.toFixed(2)}
        </span>
...
function L1Content({ colors, isDark }: { colors: Colors; isDark: boolean }) {
  ...
  function handleScreen() {
    mutate({
      ...
      operating_cashflow: [
        Number.parseFloat(form.ocf0) || 0,
        Number.parseFloat(form.ocf1) || 0,
        Number.parseFloat(form.ocf2) || 0,
      ],
      free_float_pct: Number.parseFloat(form.free_float_pct) / 100 || 0.2,
      market_cap_usd: Number.parseFloat(form.market_cap_usd) * 1_000_000 || 50_000_000,
      independent_directors_ratio:
        Number.parseFloat(form.independent_directors_ratio) / 100 || 0.33,
      ...
      going_concern_count: Number.parseInt(form.going_concern_count) || 0,
      related_party_revenue_pct: Number.parseFloat(form.related_party_revenue_pct) / 100 || 0,
    });
  }
  const inp = {
    className: "border px-2 py-1 text-xs font-mono bg-transparent w-full",
    style: { borderColor: colors.border, color: colors.text } as React.CSSProperties,
  };
  ...
  return (
    <div className="space-y-4 text-xs">
      ...
      <div>
        <div className="text-[8px] mb-1" style={{ color: colors.textSecondary }}>
          PROFITABLE YEARS (Y-3, Y-2, Y-1)
        </div>
        <div className="flex gap-3">
          {(["yr0", "yr1", "yr2"] as const).map((k, i) => (
      ...
      <div>
        <div className="text-[8px] mb-1" style={{ color: colors.textSecondary }}>
          OPERATING CASHFLOW M$ (Y-3, Y-2, Y-1)
        </div>
        <div className="grid grid-cols-3 gap-1">
          <input
            {...inp}
            value={form.ocf0}
            onChange={(e) => setF("ocf0", e.target.value)}
            placeholder="Y-3"
            type="number"
          />
          <input
            {...inp}
            value={form.ocf1}
            onChange={(e) => setF("ocf1", e.target.value)}
            placeholder="Y-2"
            type="number"
          />
          <input
            {...inp}
            value={form.ocf2}
            onChange={(e) => setF("ocf2", e.target.value)}
            placeholder="Y-1"
            type="number"
          />
        </div>
      ...
      <div className="grid grid-cols-2 gap-2">
        <div>
          ...
          <input
            {...inp}
            value={form.free_float_pct}
            onChange={(e) => setF("free_float_pct", e.target.value)}
            placeholder="25"
            type="number"
          />
        </div>
        <div>
          ...
          <input
            {...inp}
            value={form.market_cap_usd}
            onChange={(e) => setF("market_cap_usd", e.target.value)}
            placeholder="100"
            type="number"
          />
        </div>
      ...
      <div className="grid grid-cols-2 gap-2">
        <div>
          ...
          <input
            {...inp}
            value={form.independent_directors_ratio}
            onChange={(e) => setF("independent_directors_ratio", e.target.value)}
            placeholder="40"
            type="number"
          />
        </div>
        <div>
          ...
          <input
            {...inp}
            value={form.related_party_revenue_pct}
            onChange={(e) => setF("related_party_revenue_pct", e.target.value)}
            placeholder="10"
            type="number"
          />
        </div>
      ...
      <div>
        ...
        <input
          {...inp}
          value={form.going_concern_count}
          onChange={(e) => setF("going_concern_count", e.target.value)}
          placeholder="0"
          type="number"
        />
      </div>
      <div className="flex flex-wrap gap-3">
        {[
          ["has_audit_committee", "Audit Committee"],
          ["has_internal_controls", "Internal Controls"],
          ["auditor_registered", "Auditor Registered"],
        ].map(([k, label]) => (
          <label key={k} className="flex items-center gap-1 font-mono cursor-pointer">
            <input
              type="checkbox"
      ...
      {error && (
        <div className="text-[9px] font-mono" style={{ color: "#ef5350" }}>
      ...
      {data && (
        <div className="space-y-3">
          <div
            className="border p-3"
            style={{
              borderColor: decClr[data.decision],
              backgroundColor: isDark ? "#0a0a0a" : "#fff",
            }}
          >
            <div className="text-xl font-bold font-mono" style={{ color: decClr[data.decision] }}>
              {data.decision}
            </div>
            <div className="text-sm font-mono mt-1" style={{ color: colors.text }}>
              Score:{" "}
              <span className="font-bold" style={{ color: decClr[data.decision] }}>
          ...
          <div className="border p-3 space-y-1.5" style={{ borderColor: colors.border }}>
            <div
              className="text-[9px] tracking-widest font-mono mb-2"
            ...
            {Object.entries(data.score_breakdown).map(([k, v]) => {
              ...
              return (
                <div key={k} className="flex items-center gap-2 text-[9px] font-mono">
                  <span
                    className="w-20 capitalize truncate"
                    style={{ color: colors.textSecondary }}
                  >
                    {k}
                  </span>
                  <div className="flex-1 h-1.5" style={{ backgroundColor: colors.border }}>
                    <div className="h-full" style={{ width: `${v}%`, backgroundColor: c }} />
                  </div>
                  <span className="w-8 text-right" style={{ color: c }}>
                    {v.toFixed(0)}%
                  </span>
                </div>
              );
          ...
          {data.recommendations.length > 0 && (
            <div className="border p-3" style={{ borderColor: "#ffc107" }}>
              <div
                className="text-[9px] tracking-widest font-mono mb-1"
                style={{ color: "#ffc107" }}
              >
                RECOMMENDATIONS
              </div>
              {data.recommendations.map((r) => (
                <div key={r} className="text-[9px] font-mono" style={{ color: colors.text }}>
                  • {r}
                </div>
              ))}
            </div>
...
function L4Content({ colors, isDark }: { colors: Colors; isDark: boolean }) {
  ...
  const TIER_CLR: Record<number, string> = {
    0: "#4caf50",
    1: "#ffc107",
    2: "#ff9800",
    3: "#ef5350",
  ...
  return (
    <div className="space-y-4 text-xs">
      ...
      <div
        ...
        style={{ borderColor: colors.border, backgroundColor: colors.surface }}
      >
        <div className="text-[9px] font-bold font-mono" style={{ color: colors.accent }}>
        ...
        <div className="grid grid-cols-3 gap-2">
          ...
          <div>
            ...
            <input
              {...inp}
              value={stockPrior}
              onChange={(e) => setStockPrior(e.target.value)}
              type="number"
            />
          </div>
        </div>
        <button
          ...
          onClick={() =>
            stockChk.mutate({
              ...
              priorClose: Number.parseFloat(stockPrior),
            })
          }
          disabled={stockChk.isPending}
          className="w-full py-1 text-xs font-mono font-bold border"
          style={{ borderColor: colors.accent, color: colors.accent }}
        >
        ...
        {stockChk.data &&
          (() => {
            const d = stockChk.data;
            const tc = TIER_CLR[d.tier ?? 0] ?? "#888";
            return (
              <div className="space-y-1">
                <div
                  className="font-mono font-bold text-sm"
                  style={{ color: (d.pct_change ?? 0) < 0 ? "#ef5350" : "#4caf50" }}
                >
                  Δ {(d.pct_change ?? 0) >= 0 ? "+" : ""}
                  {(d.pct_change ?? 0).toFixed(2)}%
                </div>
                <div className="border p-1.5 inline-block" style={{ borderColor: tc }}>
                  <span className="font-bold font-mono" style={{ color: tc }}>
                    {d.halt_label}
                  </span>
                </div>
      ...
      >
        ...
        <div className="grid grid-cols-3 gap-2">
          <div>
            ...
            <input
              {...inp}
              value={idxVal}
              onChange={(e) => setIdxVal(e.target.value)}
              type="number"
            />
          </div>
          <div>
            ...
            <input
              {...inp}
              value={idxPrior}
              onChange={(e) => setIdxPrior(e.target.value)}
              type="number"
            />
          </div>
          <div>
            ...
            <input
              {...inp}
              value={idxTime}
              onChange={(e) => setIdxTime(e.target.value)}
              placeholder="14:30"
            />
          </div>
        </div>
        <button
          ...
          onClick={() =>
            mktChk.mutate({
              indexValue: Number.parseFloat(idxVal),
              priorClose: Number.parseFloat(idxPrior),
              time: idxTime,
            })
          }
          disabled={mktChk.isPending}
          className="w-full py-1 text-xs font-mono font-bold border"
          style={{ borderColor: colors.accent, color: colors.accent }}
        >
        ...
        {mktChk.data &&
          (() => {
            const d = mktChk.data;
            const lc = LVL_CLR[d.level ?? "NONE"] ?? "#888";
            return (
              <div className="space-y-1">
                <div className="font-mono font-bold text-sm" style={{ color: "#ef5350" }}>
                  Δ {(d.drop_pct ?? 0).toFixed(2)}%
                </div>
                <div className="border p-1.5 inline-block" style={{ borderColor: lc }}>
                  <span className="font-bold font-mono" style={{ color: lc }}>
                    {d.halt_label}
                  </span>
                </div>
                <div className="text-[9px] font-mono" style={{ color: colors.textSecondary }}>
        ...
        >
          ...
          <div className="flex items-baseline gap-3">
            <span
              ...
              style={{ color: marginData.required_margin_pct > 25 ? "#ff9800" : "#4caf50" }}
            >
              {marginData.required_margin_pct.toFixed(1)}%
            </span>
            <span
              className="border px-2 py-0.5 text-xs font-bold font-mono"
              style={{
                color:
                  marginData.level === "NORMAL"
                    ? "#4caf50"
                    : marginData.level === "ELEVATED"
                      ? "#ffc107"
                      : "#ef5350",
                borderColor:
                  marginData.level === "NORMAL"
          ...
          <div className="text-[9px] font-mono" style={{ color: colors.textSecondary }}>
            {marginData.rationale}
          </div>
          <div className="grid grid-cols-2 gap-2">
            {[
              ["CAPE", marginData.cape?.toFixed(1) ?? "N/A", marginData.cape_regime],
              ["VIX", marginData.vix?.toFixed(1) ?? "N/A", marginData.vix_regime],
            ].map(([lbl, val, reg]) => (
...
function Drawer({
  ...
  return (
    <div className="absolute inset-0 z-40 flex" style={{ pointerEvents: "all" }}>
      {/* biome-ignore lint/a11y/useKeyWithClickEvents: click-away backdrop, not a control — the panel's own buttons are keyboard-reachable on their own */}
  ...
  return (
    <div className="relative flex gap-3 h-full overflow-hidden">
      ...
      <Drawer
        open={drawer === "listing"}
        onClose={() => setDrawer(null)}
        title="IPO LISTING QUALITY GATE"
        colors={colors}
        isDark={isDark}
      >
