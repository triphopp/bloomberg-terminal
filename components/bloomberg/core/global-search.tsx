...
export const DEFAULT_WATCHLIST_GROUP: PinGroup = { id: "watchlist", name: "Watchlist", color: "#f59e0b" };
...
function typeColor(type?: string): string {
  switch ((type ?? "").toLowerCase()) {
    case "equity":     return "#4ade80";
    case "etf":        return "#60a5fa";
    case "index":      return "#a78bfa";
    case "mutualfund": return "#fb923c";
    case "currency":   return "#f59e0b";
    case "future":     return "#f87171";
    ...
    <div className="px-4 py-3 font-mono border-b" style={{ borderColor: colors.border }}>
      ...
      {c.type === "scalar" && (
        <>
          ...
          {c.sub && (
            <div className="text-[10px]" style={{ color: "#555" }}>{c.sub}</div>
          )}
        </>
      )}
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-[10px]" style={{ borderCollapse: "collapse" }}>
            <thead>
              <tr>
                {c.cols.map((col) => (
                  <th key={col} className="text-left py-0.5 px-2 font-bold"
      ...
      {c.type === "info" && (
        <div className="space-y-0.5">
          {c.lines.map((line: string, i: number) => (
            <div key={i} className="text-[10px]" style={{ color: colors.textSecondary }}>{line}</div>
          ))}
        </div>
      )}
...
function SuggestionRow({
  ...
  return (
    <div
      ...
      onClick={onSelect}
    >
      <div className="w-1 h-5 shrink-0" style={{ background: isActive ? colors.accent : "transparent" }} />
      <span className="font-bold font-mono text-xs w-44 shrink-0 truncate" style={{ color: colors.accent }}>
        {suggestion.label}
      </span>
...
function GroupPicker({
  ...
  return (
    ...
    >
      ...
      {groups.map((g) => (
        <button key={g.id} className="w-full flex items-center gap-2 px-3 py-2 text-xs hover:opacity-70"
          style={{ color: colors.text }} onClick={() => onPick(g)}>
          <span className="w-2.5 h-2.5 rounded-full shrink-0" style={{ background: g.color }} />
          {g.name}
        </button>
      ))}
      <button className="w-full text-left px-3 py-1.5 text-xs border-t"
        style={{ color: colors.textSecondary, borderColor: colors.border }} onClick={onClose}>
        Cancel
      </button>
    </div>
  );
...
function ResultRow({
  result, isActive, isPinned, onOpen, onPinClick, colors, rowRef,
}: {
  result:     SearchResult;
  isActive:   boolean;
  isPinned:   boolean;
  ...
  return (
    <div
      ...
      style={{ background: isActive ? `${colors.accent}18` : "transparent", borderBottom: `1px solid ${colors.border}` }}
      onClick={onOpen}
    >
      <div className="w-1 h-6 rounded-full shrink-0" style={{ background: isActive ? colors.accent : "transparent" }} />
      <span className="font-bold font-mono text-sm w-24 shrink-0" style={{ color: colors.accent }}>{result.symbol}</span>
      <span className="flex-1 text-sm truncate" style={{ color: colors.text }}>{name}</span>
      {result.typeDisp && (
        <span className="text-xs px-1.5 py-0.5 rounded font-bold shrink-0 hidden sm:block"
          style={{ background: `${tColor}22`, color: tColor, border: `1px solid ${tColor}44` }}>
          {result.typeDisp.toUpperCase()}
        </span>
      )}
      {result.exchDisp && (
        <span className="text-xs shrink-0 w-20 text-right hidden sm:block" style={{ color: colors.textSecondary }}>
          {result.exchDisp}
        </span>
      )}
      <button
      ...
      >
        {isPinned ? <><Check className="h-3 w-3" />PINNED</> : <><Pin className="h-3 w-3" />PIN</>}
      </button>
    </div>
  );
}
...
export function GlobalSearch() {
  ...
  const ctx: TerminalCtx = {
    ...
    invalidate:       (ks) => ks.forEach((k) => queryClient.invalidateQueries({ queryKey: [k] })),
    ...
    const t = setTimeout(async () => {
      try {
        ...
        if (
          ...
          "FEAR".startsWith(q) ||
          "GREED".startsWith(q) ||
          q === "FG" || q === "F&G" || q === "SENTIMENT"
        ) {
          syntheticResults.push({
            symbol:   "FEAR-GREED",
            shortname: "Fear & Greed Index",
            longname:  "CNN-style Fear & Greed composite (VIX, momentum, safe-haven, junk bonds, breadth)",
            typeDisp:  "Index",
          });
        }
  ...
  useEffect(() => { rowRefs.current[activeIdx]?.scrollIntoView({ block: "nearest" }); }, [activeIdx]);

  // ── Actions ────────────────────────────────────────────────────────────────

  const openEquity = useCallback((sym: string) => {
    setStockSymbol(sym);
    setCurrentView("stock" as any);
    setIsOpen(false);
  }, [setStockSymbol, setCurrentView, setIsOpen]);

  const runCommand = useCallback(async (completion?: string) => {
  ...
  const doPin = useCallback((sym: string, group: PinGroup) => {
    if (pins.some((p) => p.symbol === sym && p.groupId === group.id)) {
      setPinnedNow((s) => new Set([...s, sym]));
      return;
    }
    const newPin: PinnedAsset = {
      id: Date.now().toString(), symbol: sym, groupId: group.id,
      comment: "", addedAt: new Date().toISOString().split("T")[0],
    };
    setPins((ps) => [...ps, newPin]);
    savePin(newPin);
    setPinnedNow((s) => new Set([...s, sym]));
    setPinFeedback((fb) => ({ ...fb, [sym]: group.name }));
    setTimeout(() => setPinFeedback((fb) => { const n = { ...fb }; delete n[sym]; return n; }), 2500);
    setPickerFor(null);
  ...
  const handlePinClick = useCallback((e: React.MouseEvent, sym: string) => {
    e.stopPropagation();
  ...
  const handleKeyDown = useCallback((e: React.KeyboardEvent<HTMLInputElement>) => {
    ...
    results, activeIdx, pickerFor, isCommandMode, suggestions,
    query, runCommand, openEquity, doPin, groups, setIsOpen,
  ]);
      ...
      >
        ...
        <div className="flex items-center gap-3 px-4 py-3 border-b" style={{ borderColor: colors.border }}>
          ...
          <input
            ...
            value={query}
            onChange={(e) => { setQuery(e.target.value); setPickerFor(null); setActiveIdx(0); }}
        ...
        <div ref={listRef} className="overflow-y-auto" style={{ maxHeight: "min(65vh, 480px)" }}>
          ...
          {execResult?.kind === "error" && (
            <div className="px-4 py-3 border-b font-mono" style={{ borderColor: colors.border }}>
              <div className="text-[9px] tracking-widest mb-1" style={{ color: "#f87171" }}>ERROR</div>
              <div className="text-sm" style={{ color: "#f87171" }}>{execResult.message}</div>
            </div>
          )}
          ...
          {execResult?.kind === "display" && (
            <ResultPanel
              ...
              colors={colors}
              onClose={() => { setExecResult(null); setQuery(""); inputRef.current?.focus(); }}
            />
          )}
                  ...
                  {["analysis", "nav", "setting", "info"].map((group) => {
                    ...
                    return (
                      <div key={group} className="mb-3">
                        <div className="text-[9px] font-bold tracking-widest mb-1 uppercase"
                          style={{ color: gc }}>{group}</div>
                        {cmds.map((c) => (
                          <div key={c.name} className="flex gap-3 text-[10px] py-0.5">
                            <span className="w-40 shrink-0 font-bold" style={{ color: colors.accent }}>
                              {c.name}
                              {c.args?.length ? `(${c.args.map(a => a.optional ? `${a.name}?` : a.name).join(", ")})` : ""}
                            </span>
            ...
            <>
              {!query.trim() && (
                <div className="py-10 text-center" style={{ color: colors.textSecondary }}>
                  ...
                  <div className="text-xs mt-2 opacity-60">AAPL · TSLA · ^GSPC · ^SET.BK · GC=F</div>
                </div>
              )}
              {query.trim() && !loading && searchError && (
                <div className="py-10 text-center px-6">
                  <div className="text-sm font-bold mb-1" style={{ color: "#f87171" }}>Search failed</div>
                  <div className="text-xs" style={{ color: colors.textSecondary }}>{searchError}</div>
                </div>
              )}
        ...
        >
          <span><kbd className="px-1.5 py-0.5 rounded text-xs font-mono" style={{ background: colors.border }}>↑↓</kbd> navigate</span>
          <span><kbd className="px-1.5 py-0.5 rounded text-xs font-mono" style={{ background: colors.border }}>Tab</kbd> complete</span>
          <span><kbd className="px-1.5 py-0.5 rounded text-xs font-mono" style={{ background: colors.border }}>↵</kbd> execute / open</span>
          <span><kbd className="px-1.5 py-0.5 rounded text-xs font-mono" style={{ background: colors.border }}>Ctrl+P</kbd> pin asset</span>
          <span><kbd className="px-1.5 py-0.5 rounded text-xs font-mono" style={{ background: colors.border }}>Esc</kbd> close</span>
