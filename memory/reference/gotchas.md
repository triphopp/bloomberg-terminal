...
| TAIL ขึ้น NORMAL ทั้งที่ MOVE +17.5% ในวันเดียว (2026-09-23) | `_vol_signal_frame` reindex ทุกสัญญาณเข้า**ปฏิทิน VIX** — CBOE ออก VIX ช้ากว่า Yahoo 1 session แท่งใหม่สุดของ MOVE (Yahoo) จึงถูกทิ้ง และวันที่ Yahoo ไม่มีแท่ง MOVE ถูก `fill_value=False` = off | **FIXED 2026-09-24** — union calendar + `ffill(limit=_CARRY_BARS)`; test `test_move_bar_newer_than_vix_is_not_dropped`. **กฎ:** ห้าม align หลายแหล่งเข้าปฏิทินของแหล่งเดียว ถ้าแต่ละแหล่งออกข้อมูลคนละเวลา |
| สัญญาณ "ระดับเกินเกณฑ์" ยิงแทบทุกวัน (SKEW > 145) | เกณฑ์ absolute ตั้งไว้ตอน regime อื่น — 2026-09 SKEW 145 คือ **median 1 ปี** | อ่านเทียบช่วงของตัวเอง (z63) + backstop absolute ที่สูงกว่าทุกค่าที่เห็นล่าสุด (`SKEW_EXTREME` 160 ใน `tail_events.py`). ⚠️ `skew_elevated` ใน 6 มิติยังใช้ >145 (มี backtest ไม่แตะ) |
| ช็อกช่องทางเดียวรุนแรงแต่ composite ไม่ขยับ | gate ต้อง 2 สัญญาณ/มิติ × 2 มิติ และทุกสัญญาณวัด z ของ**ระดับ** ไม่มีตัวไหนวัดขนาด*การเปลี่ยนแปลง* | `tail_events.py` — z ของ 1d/5d change เทียบ 252 changes ของตัวเอง, ตั้งชื่อเหตุการณ์ + event floor ต่อ channel (2026-09-24) |
| แยก nominal move ผิดข้าง — real-rate day ถูกเรียกว่า inflation scare | DFII10 (FRED) ออกช้ากว่า ^TNX 1 session; อ่าน as-of แล้วเอา Δreal ของ *เมื่อวาน* (+1bp) ไปเทียบ Δnominal ของ*วันนี้* (+15bp) | **FIXED 2026-09-24** — `decompose()` ต้องวันที่ตรงกันทั้ง 3 leg; real yield ต่อถึงวันล่าสุดด้วย identity (anchor วันล่าสุดที่ทั้ง 3 มีค่า — Yahoo ข้ามวันได้). **กฎ:** series ที่อ่านแบบ as-of ห้ามเอามาหักลบกับ series รายวันโดยไม่เช็ควันที่ |
| Brent−WTI / crack spread "ช็อก" วันเดียวแล้วหาย | Yahoo continuous futures (CL=F, BZ=F, HO=F, RB=F) เปลี่ยนสัญญาคนละวัน — 2026-09-23 CL=F roll Oct→Nov ทำ Brent−WTI +$6.26 (z 4.4) ปลอม, 2026-09-01 month-start roll ทำ crack "ยุบ" SEVERE | **FIXED 2026-09-24** — `ROLL_LEGS` + `wti_roll_dates()` / `month_start_roll_dates()` ใน `tail_events.py` blank change ที่ window คร่อมวัน roll (1d และ 5d). RBOB ยังเปลี่ยนเกรดฤดูกาล เม.ย./ก.ย. — ไม่ได้ mask |
| TAIL: "EVENT CLASSIFIER NO DATA" (2026-09-24 19:57) | Yahoo 429 ทั้ง IP (log ขึ้น `possibly delisted; no price data found` = คำตอบว่าง ไม่ใช่ delisted จริง) + DNS หลุดสั้นๆ (`Failed to resolve api.stlouisfed.org`). ไม่มี last-good → pull พลาดครั้งเดียว panel หายทั้งอัน; FRED fetch คืน `{}` ซึ่ง TTLCache นับเป็น hit → ค้าง 1 ชม. | **FIXED** — `_remember/_recall` ใน `tail_risk.py` (last-good ≤36h, เขียน `backend/cache/tail_*.pkl` รอด restart) ใช้กับ cross-asset panel, SPY/AGG, DCC, FRED; payload `events_stale_hours`; history 2y ดึงทุก 6h + refresh 5d ทุก ~5 นาที (ลดโหลด Yahoo). **กฎ:** compute ที่ล้มต้องคืน `None` ไม่ใช่ container ว่าง; อย่า print exception ของ `_fred_fetch` ตรงๆ — ข้อความมี URL + api_key |
| เน็ตบ้านหลุดหลังเปิดเว็บ (มือถือ 4G ยังใช้ได้), log มี `Failed to resolve` + Yahoo 429 | วัด 2026-09-24: เปิดเว็บ = **228 connection ใหม่ใน 10 วิแรก, peak 109 socket** เกือบทั้งหมดไป Yahoo — ทุก router ดึงเองพร้อมกัน + `yf.download` เปิด 1 thread ต่อ ticker | `backend/yahoo_gate.py` ครอบ `yfinance.data.YfData._make_request` ด้วย semaphore (≤6 in-flight ทั้ง process) + `threads=4` ใน tail_risk/rotation/fear_greed → วัดซ้ำ 86 ใน 10 วิแรก peak 70 (นับรวม TIME_WAIT). ผลข้างเคียง: cold `/api/tail-risk/signals` ~70 วิ → proxy timeout ขึ้นเป็น 180s. ยังไม่พิสูจน์ 100% ว่าเป็นสาเหตุเน็ตหลุด — ถ้ายังหลุด ลด `YAHOO_MAX_CONCURRENT` |
| (2026-09-24 ต่อ) ผู้ใช้เปลี่ยนใจ: **ไม่แสดง alert บน UI** | — | bar ถูกลบ; เหตุการณ์ลง `logs/upstream.jsonl` แทน → agent ตรวจด้วย `python backend/scripts/upstream_report.py` (ขั้นตอนใน CLAUDE.md) |
| แหล่งข้อมูล/เน็ตล่มแต่แอปเงียบ — แต่ละ panel ว่างแยกกัน ไม่มีใครบอกต้นเหตุ | ไม่มีจุดรวมสถานะ outbound; log ของ yfinance พิมพ์แค่ `possibly delisted` | **FIXED 2026-09-24** — `upstream_health.py` บันทึกทุก call (requests + yfinance) → `GET /api/health/upstream` → `UpstreamAlert` bar. **กฎ:** fetch ใหม่ที่ใช้ `requests`/yfinance ถูกนับอัตโนมัติ; ถ้าใช้ client อื่น (httpx, aiohttp, curl_cffi ตรง) ต้องเรียก `upstream_health.record(source, kind)` เอง; ถ้า serve ข้อมูลเก่าให้ใช้ `last_good.cached_fetch/recall` เพื่อให้ขึ้นเตือน stale |
| FRED `Read timed out (read timeout=15)` / 502 เป็นช่วงๆ → bar เหลืองขึ้นบ่อย | startup ยิง FRED พร้อมกัน 20+ (rates 11 tenor/6 thread, crisis 8, macro 4, calendar 4, TAIL) ไม่มี retry — ช้าครั้งเดียว = fail = alert แม้ call ถัดไปผ่าน | **FIXED 2026-09-24** ใน `upstream_health.install()` (hook ของ `requests`): FRED ≤3 พร้อมกัน, GET retry 2 ครั้ง (1s, 3s) เฉพาะ timeout/connection/5xx/429 — **ไม่ retry DNS**, ไม่ retry POST, หยุด retry เมื่อแหล่งนั้นพลาด 3 ครั้งติดใน 60s (fail-fast); นับเฉพาะผลสุดท้าย (`retries_window` แยก); DEGRADED ต้อง ≥2 final failures/10 นาที. วัดหลังแก้: startup retry ช่วยไว้ 2, final fail 1 → overall OK |
| FRED `api_key` โผล่ใน `logs/backend.log` | exception ของ requests quote URL เต็ม และโค้ดหลายที่ `print(f"...{exc}")` | **FIXED 2026-09-24** — hook redact `api_key/apikey/token/key=` ใน exception args และ `response.url` (ที่ `raise_for_status` ใช้) ก่อนส่งกลับ; log เก่า 19 จุดถูก mask ด้วย `*` แล้ว (2026-09-24, แทนที่ byte-for-byte ขณะ backend ยังเขียนอยู่) |
| "FRED สะดุด" ขึ้นซ้ำทุกไม่กี่นาที | **ไม่ใช่ FRED ช้า** — FRED ลบ series `BAMLHE00EHY0D` (EM HY OAS, `em_hy_spread` ใน crisis.py): JSON API ตอบ 400 "series does not exist" → fallback `fredgraph.csv` แขวนจน timeout 5s → retry → ~20s → ไม่ cache อะไร → call ถัดไป (~70s) เริ่มใหม่. วัด: FRED JSON ปกติตอบ 0.7–1.8s | **FIXED 2026-09-24** — `crisis._miss_until`: series ที่ว่างจากทุกแหล่ง ไม่ขอซ้ำ 6h (test `test_crisis_miss_backoff.py`); หา target ได้จาก `recent_retries`/`incidents[].targets` ใน `/api/health/upstream` + log `[upstream] FRED …`. EM HY OAS ถูกลบออกจาก crisis config + CRDT แล้ว (2026-09-24). **กฎ:** fetch ที่ fail ต้องมี negative cache — ไม่งั้น "ว่าง = หมดอายุ" = ยิงซ้ำทุก request |
...
## z-score ของ series แบน: sd ไม่เท่า 0 (2026-09-23)

`_z_last()` ใน `routers/rotation.py` เคยกันแค่ `if sd <= 0: return None` — ไม่พอ
series ที่ค่าคงที่ทุกวัน (เช่นส่วนแบ่งมูลค่าซื้อขายที่ไม่ขยับ) ให้ `std() ≈ 1e-17` ไม่ใช่ 0
เพราะ mean กับค่าแต่ละตัวหักล้างกันไม่ลงตัวใน floating point แล้ว `(x − mean) / 1e-17`
คืน **±1.0 เป๊ะ** — series ที่นิ่งสนิทกลายเป็นสัญญาณ 1σ ทุกครั้ง (test จับได้ตอนเขียน)

```python
if not (sd > max(abs(mean), 1e-12) * 1e-9):   # floor สัมพัทธ์กับ level ที่กำลัง score
    return None
```

**กฎ:** ทุกที่ที่หารด้วย sd/σ ที่คำนวณจากข้อมูลจริง ต้องมี floor สัมพัทธ์ ไม่ใช่ `> 0`
โดยเฉพาะเมื่อค่าที่ score เป็นเศษส่วนของ 1 (share, weight, ratio) ซึ่ง sd จริงก็เล็กอยู่แล้ว

## z ของ "ระดับ" กับ z ของ "การเปลี่ยนแปลง" ตอบคนละคำถาม (2026-09-23)

SECTOR ROTATION ใน TAIL: ส่วนแบ่ง turnover ของ XLK อยู่ราว 10-12% แทบตลอดปี
